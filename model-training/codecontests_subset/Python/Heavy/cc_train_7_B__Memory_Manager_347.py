...
"""
def alloc(search_start_index, size):
    limit = len(memory) - size + 2# bc if we got to len(test)-3 + 1 withut finding zero elements, that's it!

    count = 0
    start_index = search_start_index
    found = False

    for l in range(search_start_index, len(memory)):
        if memory[l] == 0:
            count += 1

            if count == 1:#first zero element
                start_index = l
            ...
            no_zeros = True
            for xl in range(l, limit):
                if memory[xl] == 0:
                    return alloc(xl, size)
                    no_zeros = False
    ...
    if found:
        return (start_index + 1) #start_index + 1 in case of start_index being zero
    else:
        return False
...
def erase(identifier, m):
    erase_counter = 0
    for j in range(m):
        if memory[j] == identifier:
            memory[j] = 0
            erase_counter += 1
    return erase_counter

def defragment(memory):
    temp = [k for k in memory if k]
    temp += [0] * (m - len(temp))
...
for i in range(t):
    ...
    if op == 'alloc':
        if arg <= empty_elm_counter:
            test1 = alloc(0, arg)
            if test1:
                empty_elm_counter -= arg
                for xy in range((test1 - 1), ((test1 - 1) + arg)):
                    memory[xy] = identifier_count
