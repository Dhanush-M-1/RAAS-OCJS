...
# ***********Here Basically we need to find the level of nodes************
def depthofvertex(array, level, d):
    if not array:
        return 
    maximum = max(array)
    d[maximum] = level
    index = array.index(maximum)
    depthofvertex(array[:index], level+1,d)
    depthofvertex(array[index+1:], level+1,d)
for _ in range(int(input())):
    n = int(input())
    array = [int(x) for x in input().split()]
    d = dict()
    depthofvertex(array, 0, d)
