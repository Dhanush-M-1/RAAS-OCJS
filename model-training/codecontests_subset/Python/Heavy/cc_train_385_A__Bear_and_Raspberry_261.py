...
def binary(n): return bin(n)[2:]


def decimal(s): return (int(s, 2))
...
def ncr(n,r): return M.factorial(n) // (M.factorial(n - r) * M.factorial(r))


def npr(n,r): return M.factorial(n) // M.factorial(n - r)
...
def YES(c): print(IF(c, "YES", "NO"))

def Yes(c): print(IF(c, "Yes", "No"))

def yes(c): print(IF(c, "yes", "no"))

def JA(a, sep=" "): print(sep.join(map(str, a)))

def JAA(a, s='\n', t=" "): return s.join(t.join(map(str, b)) for b in a)

def PS(a,s=" "): print(str(a),end=s) 
...
def MATI(n): return [ LI() for i in range(n)]

def MATS(n): return [list(inp()) for i in range(n)]

def IV(): return map(int,inp().split())

def FV(): return map(float,inp().split())

def LS(): return list(inp())
...
def solve():
    ...
    print(ma-r) if ma-r>0 else print(0)
