...
class Serialization:
    ...
    def __getitem__(self, item):
        ...
        if last > LOAD:
            if isinstance(item, slice):
                ...
                return list(map(self.__call__, range(start, stop, step)))
    ...
    def resize_with_value(self, value):
        ...
        while self.data[-1] < value:  # or here, or either
    ...
    class Seq(Serialization):
        ...
        def resize(self, n):
            ...
            for i in range(length, n):  # formula here
                self.data[i] = (self.A000142[i] - self.A011782[i - 1]) % MOD
