...
def main():
    ...
    nCr = make_nCr_mod(3 * (10 ** 5) + 1, MOD)

    n, k = map(int, input().split())
    lamps = sorted([[int(i) for i in input().split()] for _ in range(n)], key=lambda x: x[0])

    off_times = SortedList([])
    sol = 0

    for lamp in lamps:
        removed = 0
        while len(off_times) and off_times[0] < lamp[0]:
            off_times.pop(0)
            removed += 1

        term = 0
        for i in range(1, min(k, removed) + 1):
            if k - i > len(off_times):
                continue
            term += (nCr(removed, i) * nCr(len(off_times), k - i)) % MOD

        sol = (sol + term) % MOD
        off_times.add(lamp[1])

    if len(off_times) >= k:
        sol = (sol + nCr(len(off_times), k)) % MOD
