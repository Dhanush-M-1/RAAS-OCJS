			...
			try {
				...
				if (arr[0] == '+')
					...
					for (int i = 0; i < arr.length; i++)
						if (arr[i] == ':') {
							ret += n * (arr.length - i - 1);
							// out.printLine(i, arr.length);
						}
			} catch (Exception ex) {
				break;
			}
...
class B implements Task {
	...
	public void solve(int testNumber, InputReader in, OutputWriter out) {
		...
		for (int i = 0; i < n; i++) {
			...
			if (t == 2)
				l1.add(new IntPair(-p, i + 1));
			else
				l2.add(new IntPair(-p, i + 1));
		}
		Collections.sort(l1);
		Collections.sort(l2);
		...
		arr = new int[i1 + i2];
		for (int i = 0; i < i1; i++)
			arr[i] = l1.get(i).second;
		for (int i = 0; i < i2; i++)
			arr[i + i1] = l2.get(i).second;
		out.printLine(arr);
	}
...
class C implements Task {
	...
	public void solve(int testNumber, InputReader in, OutputWriter out) {
		...
		// out.printLine(vx?"true":"false", vy?"true":"false");
		if (cx > cy + 1 || cy > cx || vx && vy || vy && cx > cy || vx
				&& cx == cy)
			out.printLine("illegal");
		else if (cx + cy == 9 && !vx && !vy)
			out.printLine("draw");
		else if (!vx && !vy)
			out.printLine(cx > cy ? "second" : "first");
		else
			out.printFormat("the %s player won\n", vx ? "first" : "second");
	}
	...
	int pd(int mask) {
		...
		for (int i = 0; i < n; i++)
			if ((mask & (1 << i)) > 0) {
				...
				memo[mask] = MiscUtils.min(memo[mask], soma - c + pd(mask2));
			}
		return memo[mask];
	}
