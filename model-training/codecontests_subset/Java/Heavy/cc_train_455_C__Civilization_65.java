...
public class cf455c_2 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for(int i = 0; i < n; ++i) {
            if(dsu.dia[dsu.find(i)] == -1) {
                dsu.dia[dsu.find(i)] = g[i].dia(null).a;
            }
        }
        /*for(int i = 0; i < n; ++i) {
            prln(i, dsu.dia[dsu.find(i)]);
        }*/
        while(q --> 0) {
            int t = rni();
            if(t == 1) {
                prln(dsu.dia[dsu.find(ni() - 1)]);
            } else {
                dsu.union(ni() - 1, ni() - 1);
            }
        }
    ...
    static class Node {
        ...
        P dia(Node par) {
            if(par == null && nei.size() == 0 || par != null && nei.size() == 1) {
                return new P(0, 0);
            } else {
                PriorityQueue<Integer> lens = new PriorityQueue<>((a, b) -> b - a);
                ...
                P p = new P(max(ans, left + right + min(nei.size() - (par == null ? 0 : 1), 2)), left + 1);
                /*pr(i);
                pr(' ');
                prln(p);*/
                return p;
    ...
    static class DSU {
        ...
        void union(int u, int v) {
            ...
            if(a != b) {
                ...
                par[b] = a;
                sz[a] += sz[b];
                dia[a] = maxof(dia[a], dia[b], (dia[b] + 1) / 2 + (dia[a] + 1) / 2 + 1);
            }
        }
    }
