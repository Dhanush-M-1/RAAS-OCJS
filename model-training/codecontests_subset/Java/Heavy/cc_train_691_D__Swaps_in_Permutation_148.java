...
public class _0691_D_SwapsInPermutation {
	...
	public static void main(String[] args) throws IOException {
		...
		for(int j = 1; j<=M; j++) {
			...
			int ap = find(par, a), bp = find(par, b);
			if(ap == bp) continue;
			par[ap] = bp;
		}
		PriorityQueue<Integer> pq[] = new PriorityQueue[N+1];
		for(int i = 1; i<=N; i++) pq[i] = new PriorityQueue<>(Collections.reverseOrder());
		for(int i = 1; i<=N; i++) pq[find(par, i)].add(arr[i]);
		for(int i = 1; i<=N; i++) print(pq[find(par, i)].poll() + " ");
		exit();
	}
