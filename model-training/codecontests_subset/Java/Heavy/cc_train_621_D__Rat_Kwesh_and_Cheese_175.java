...
public class D implements Runnable{
    ...
    static class Expression {
        ...
        public Expression(String x, String y, String z, boolean brackets) {
            this.x = x;
            this.y = y;
            this.z = z;
        ...
        double evaluate(Map<String, Double> values, boolean allLessOne) {
            double xValue = values.get(x);
            if (allLessOne && xValue < 1) xValue = 1 / xValue;

            double yValue = values.get(y);
            double zValue = values.get(z);

            double logX = Math.log(xValue);
            double logLogX = Math.log(logX);

            double logY = Math.log(yValue);

            if (brackets) {
                double logZ = Math.log(zValue);
                return (logZ + logY + logLogX);
            } else {
                return zValue * logY + logLogX;
            }
        }

        @Override
        public String toString() {
            String inner = x + POWER + y;
            if (brackets) {
                inner = "(" + inner + ")";
            }

            return inner + POWER + z;
        }
    }
    ...
    private void solve() {
        ...
        addExpressions(expressions, X, Y, Z);
        addExpressions(expressions, Y, X, Z);
        addExpressions(expressions, Z, X, Y);
        ...
        double bestExpressionValue = Double.NaN;

        for (Expression expression : expressions) {
            double value = expression.evaluate(values, allLessOne);
            if (Double.isNaN(value)) continue;
            
            if (null == bestExpression) {
                bestExpression = expression;
                bestExpressionValue = value;
            } else if (allLessOne && bestExpressionValue > value) {
                bestExpression = expression;
                bestExpressionValue = value;
            } else if (!allLessOne && bestExpressionValue < value) {
                bestExpression = expression;
        ...
        out.println(bestExpression);
    }

    void addExpressions(List<Expression> expressions, String x, String y, String z) {
        addExpressions(expressions, x, y, z, false);
        addExpressions(expressions, x, y, z, true);
    }

    void addExpressions(List<Expression> expressions, String x, String y, String z, boolean brackets) {
        expressions.add(new Expression(x, y, z, brackets));
        expressions.add(new Expression(x, z, y, brackets));
    }
