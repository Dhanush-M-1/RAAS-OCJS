...
public class TimePass implements Runnable {
    ...
    static void dfs(int u,int[][] g,int par)
    {
    	//System.out.println(u+"u:"+sum[u]);
    	...
    	for(int v:g[u])
    	{
    		min=Math.min(min, sum[v]);
    	}
    	if(sum[u]==-1 && par!=-1 && min!=-1)
    	{
    		sum[u]=min;
    		a[u]=min-sum[par];
    	}
	...
	static long fnc(int x,int y)
    {
    	return x+(long)1000000007*y;
    }
