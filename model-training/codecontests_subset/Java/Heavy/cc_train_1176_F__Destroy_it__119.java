...
public class cf1176f {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            int k = ri();
            // dp2[doubled][#cards][cost]
            long dp2[][][] = new long[2][4][4];
            for (long[][] layer : dp2) {
                for (long[] row : layer) {
                    fill(row, -LMAX / 2);
                }
            ...
            for (int ind = 0; ind < k; ++ind) {
                for (int jnd = min(3, k) - 1; jnd >= 0; --jnd) {
                    for (int knd = 3 - c[ind]; knd >= 0; --knd) {
                        dp2[0][jnd + 1][knd + c[ind]] = max(dp2[0][jnd + 1][knd + c[ind]], dp2[0][jnd][knd] + d[ind]);
                        dp2[1][jnd + 1][knd + c[ind]] = maxof(dp2[1][jnd + 1][knd + c[ind]], dp2[0][jnd][knd] + 2L * d[ind], dp2[1][jnd][knd] + d[ind]);
                    }
                }
            }
            /* for (int j = 1; j <= 3; ++j) {
            ...
            for (int j = 0; j < 10; ++j) {
                ndp[j] = dp[j];
                for (int ind = 1; ind <= min(3, k); ++ind) {
                    if (j - ind < 0) {
                        ndp[j] = max(ndp[j], dp[(j - ind + 10) % 10] + maxof(dp2[1][ind]));
                    } else {
                        ndp[j] = max(ndp[j], dp[j - ind] + maxof(dp2[0][ind]));
                    }
                }
            }
            dp = ndp;
