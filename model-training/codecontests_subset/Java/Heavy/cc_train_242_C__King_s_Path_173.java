...
//			return (x*737+weight).hash;
	...
	public static boolean valid(int x, int y) {
		pair p=new pair(x,y);
		return (x > 0 && x <= 1e9 && y > 0 && y <=1e9&&hm1.containsKey(x*371+773*y)&&hm1.get(x*371+773*y)==INF );
	}
...
//		if(!hm.get(u).containsKey(v))return -1;
	     hm1.put(p.x*371+p.weight*773, 0);
		...
		while (!q.isEmpty()) {
			...
			int x=pair.x;
			int y=pair.weight;
			if(x==tx&&y==ty)return hm1.get(pair.x*371+pair.weight*773);
			for (int i=0;i<8;i++) {
				int x1=x+dx[i];
				int y1=y+dy[i];
				if (valid(x1, y1)) {
					...
					hm1.put(p2.x*371+p2.weight*773, hm1.get(pair.x*371+pair.weight*773)+1);
				}
			}
		}
		return -1;
	...
	//	hm= new HashMap<Integer, HashMap<Integer,Integer>>();
		...
		hm1=new HashMap<Integer, Integer>();
