...
int n= scn.nextInt() ;
long[] tag= new long[n] ;long[] s= new long[n] ;long[] dp= new long[n] ;

for(int i=0; i < n;i++)tag[i] = scn.nextLong() ;
for(int i=0; i < n;i++)s[i] = scn.nextLong() ;
...
for(int i = 0 ; i < n ;i++)
{
    ...
    for(int j =i-1 ; j>= 0 ; j--)
    {
        ...
        long dpi = dp[i] ; long dpj = dp[j] ;long jump = s[i]-s[j] ; if(jump < 0) jump = -jump ;
        
         dp[j] = Math.max(dp[j] , dpi+jump) ;
         dp[i] = Math.max(dp[i] , dpj + jump) ;
    }
