...
public class Main {
    ...
    public static void main(String[] args)
    {
        ...
        for (int i =0;i<=k-n;i++)
        {
            ...
            for (int j=0;j<n;j++)
            {
                min=Math.max(min,Math.abs(b[i+j]-a[j])+Math.abs(b[i+j]-p));
            }
    ...
    static void MergeSort(long[] a, int[] b,int[]c,  int p, int r)
    {
        if (p < r)
        {
            int q = (r + p) / 2;
    ...
    static void Merge(long[] a, int[] b,int[]c,int p, int q, int r)
    {
        int n1 = q - p + 1;
        int n2 = r - q;
