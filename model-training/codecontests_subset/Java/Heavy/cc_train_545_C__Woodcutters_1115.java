...
//			return a - p.a;
	...
	public static void main(String[] args) throws IOException 
	{
		...
		for(int i = 0;i<n;i++)
		{
			if(prev <(l.get(i).first- l.get(i).second))
			{
				prev = l.get(i).first;
				++c;
			}
			else if(i == n-1 || (l.get(i).first + l.get(i).second) < l.get(i+1).first)
			{
				prev = l.get(i).first + l.get(i).second;
				++c;
			}
			else {
				prev = l.get(i).first;
