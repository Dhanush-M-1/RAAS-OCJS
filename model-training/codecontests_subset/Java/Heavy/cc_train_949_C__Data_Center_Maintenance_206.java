    ...
    static class TaskC {
        public void solve(int testNumber, InputReader in, OutputWriter out) {
            ...
            for (int iter = 0; iter < m; iter++) {
                ...
                int c2 = in.readInt() - 1;
                if ((u[c1] + 1) % h == u[c2]) {
                    graph.addSimpleEdge(c1, c2);
                }
                if ((u[c2] + 1) % h == u[c1]) {
                    graph.addSimpleEdge(c2, c1);
                }
            }
            StronglyConnectedComponents stronglyConnectedComponents = new StronglyConnectedComponents(graph);
            int[] toComponent = stronglyConnectedComponents.kosaraju();
            int numComponents = ArrayUtils.maxElement(toComponent) + 1;
            boolean[] isOkComponent = new boolean[numComponents];
            Arrays.fill(isOkComponent, true);
            ...
            for (int node = 0; node < toComponent.length; node++) {
                ...
                for (int i = graph.firstOutbound(node); i != -1; i = graph.nextOutbound(i)) {
                    ...
                    if (toComponent[dest] != component) {
                        isOkComponent[component] = false;
                    }
                }
            }
            int res = -1;
            for (int ansIdx = 0; ansIdx < isOkComponent.length; ansIdx++) {
                if (isOkComponent[ansIdx]) {
