...
public class CodechefCook
{
    void run() {
        ...
        while(tc-->0) {
            ...
            int i=0,j=0;
            while(i<n && j<m) {
                if(a[i]==0) { k++; i++; sb.append("0 "); }
                else if(b[j]==0) { k++; j++; sb.append("0 "); }
                else if(a[i]<=b[j] && k>=a[i]) sb.append(a[i++]).append(" ");
                else if(k>=b[j]) sb.append(b[j++]).append(" ");
                else {
                    sbb.append("-1\n"); continue xD;
                }
            }
            while(i<n) {
                if(a[i]==0) { k++; i++; sb.append("0 "); }
                else if(a[i]<=k) sb.append(a[i++]).append(" ");
                else {
                    sbb.append("-1\n");
                    continue xD;
                }
            ...
            while(j<m) {
                if(b[j]==0) { k++; j++; sb.append("0 "); }
                else if(b[j]<=k) sb.append(b[j++]).append(" ");
                else {
                    sbb.append("-1\n");
                    continue xD;
                }
