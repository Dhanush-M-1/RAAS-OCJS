    ...
    {        
       ...
       for (int i = 0; i < q; i++) {
           ...
           if (!xmap.containsKey(points[i][0]))
               xmap.put(points[i][0], new ArrayList<>());
           xmap.get(points[i][0]).add(i);
           
           if (!ymap.containsKey(points[i][1]))
               ymap.put(points[i][1], new ArrayList<>());
           ymap.get(points[i][1]).add(i);  
       }
       
       DSU dsu = new DSU(q);
       for (List<Integer> lst : xmap.values()) {
           int size = lst.size();
           if (size > 1) {
               int p = lst.get(0);
       ...
       for (List<Integer> lst : ymap.values()) {
           int size = lst.size();
           if (size > 1) {
               int p = lst.get(0);
               for (int i = 1; i < size; i++) {
       ...
       int ans = dsu.nGroup - 1 + n + m - xmap.size() - ymap.size();
       sb.append(ans+"\n");
       //sb.append(f(n) + "\n");
       //System.out.println();
