		    ...
		    if(type.equals("alloc")){
			    ...
			    for(int i = 0; i <= mem.length - size; i++) {
				    ...
				    for(int j = i; j < i + size; j++) {
					    if(mem[j] != Long.MAX_VALUE) possible = false;
				    }
				    if(possible){
					    worked = true;
			    ...
			    Arrays.fill(newmem, Long.MAX_VALUE);
			    int pointer = 0;
			    for(int i = 0; i < mem.length; i++)  if (mem[i] != Long.MAX_VALUE){
