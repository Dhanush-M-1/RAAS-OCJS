...
public class cf1208d {
    ...
    public static void main(String[] args) throws IOException {
        int n = ri(), ans[] = new int[n];
        long a[] = rla(n);
        sgt sgt = new sgt(a, Long::sum, 0);
        for (int i = 0; i < n; ++i) {
            int ind = sgt.qry();
            // prln(ind);
            // flush();
    ...
    static class sgt {
        ...
        int qry(int node, int l, int r) {
            push(node, l, r);
            if (l == r) {
                // prln(node, l, r, sgt[node]);
                return sgt[node] == 0 ? l : -1;
            } else {
                int m = l + (r - l) / 2;
                push(node << 1, l, m);
                push(node << 1 | 1, m + 1, r);
                if (sgt[node << 1 | 1] == 0) {
                    return qry(node << 1 | 1, m + 1, r);
                } else {
                    return qry(node << 1, l, m);
                }
            }
        }
        ...
        void pull(int node) {
            sgt[node] = min(sgt[node << 1], sgt[node << 1 | 1]);
        }
    }
