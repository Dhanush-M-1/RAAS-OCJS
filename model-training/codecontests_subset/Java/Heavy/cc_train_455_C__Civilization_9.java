...
public class _455C {
    ...
    static int[] collectTmp;
    static int collectCnt;
    ...
    static void collect(int i) {
        collectCnt = 0;
        i = find(i);
        collectTmp[collectCnt++] = i;
        for (int j = next[i]; j != i; j = next[j])
            collectTmp[collectCnt++] = j;
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < n; i++)
            if (dist[i] == 0) {
                ...
                dfs(i, -1, 1, dist);
                int maxNode = i;
                for (int j=0; j<collectCnt; j++) {
                    int node = collectTmp[j];
