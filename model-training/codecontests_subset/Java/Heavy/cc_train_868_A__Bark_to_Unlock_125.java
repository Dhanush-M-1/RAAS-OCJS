...
public class A_438
{
	...
	public static class Vertex<TypeVertex extends Vertex<TypeVertex, TypeEdge>, TypeEdge extends Edge<TypeVertex, TypeEdge>>
	...
	public static void nextInts(int n, int offset, int[]... result)
			throws IOException
	{
		for (int index = 0; index < n; index++)
		{
			...
			{
				result[value][index] = nextInt() + offset;
			}
		}
	}
	...
	public static int[] nextInts(int n, int offset) throws IOException
	{
		int[] result = new int[n];
		nextInts(n, offset, result);
		return result;
	}
	...
	public static char character(String string, int x)
	{
	...
	public static void solve() throws IOException
	{
		...
		int n = nextInt();
		String[] words = nextStrings(n);
		boolean result = false;
		for (String word0 : words)
		{
			result |= word0.equals(password);
			for (String word1 : words)
			{
				result |= character(password, 0) == character(word0, 1) && character(password, 1) == character(word1, 0);
			}
		}
