...
public class InVoker {
	...
	void main() {
		...
		while(t-->0) {
			...
			}else if(s[n-1].charAt(n-2)==s[n-2].charAt(n-1)){
				end=s[n-1].charAt(n-2);
				start= end=='1'?'0':'1';
			}
			if(s[0].charAt(1)!=start) {
				x[c]=1;
				y[c++]=2;
			}
			if(s[1].charAt(0)!=start) {
				x[c]=2;
				y[c++]=1;
			}
			if(s[n-1].charAt(n-2)!=end) {
				x[c]=n;
				y[c++]=n-1;
			}
			if(s[n-2].charAt(n-1)!=end) {
				x[c]=n-1;
				y[c++]=n;
			}
			out.println(c);
			for(int i=0;i<c;i++) out.println(x[i]+" "+y[i]);
		}
