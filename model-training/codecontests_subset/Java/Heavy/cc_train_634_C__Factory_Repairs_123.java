...
public class Main {
    ...
    static class TaskD {
        public void solve(int testNumber, InputReader in, OutputWriter out) {
            ...
            int k = in.readInt(); // time to repair
            int laterRate = in.readInt(); // a
            int currentRate = in.readInt(); // b
            int numQueries = in.readInt();
            Special a = new Special(n, currentRate);
            Special b = new Special(n, laterRate);
            for (int query = 0; query < numQueries; query++) {
                int type = in.readInt();
                if (type == 1) {
                    int day = in.readInt() - 1;
                    int delta = in.readInt();
                    a.update(day, day, delta);
                    b.update(day, day, delta);
                } else if (type == 2) {
                    int startRepairs = in.readInt() - 1;
                    out.printLine(a.query(0, startRepairs - 1) + b.query(startRepairs + k, n - 1));
                } else throw new RuntimeException();
            }
        }

        public class Special extends LongIntervalTree {
            int limit;

            public Special(int size, int limit) {
                super(size);
                this.limit = limit;
            }
            ...
            protected long accumulate(long value, long delta, int length) {
                ...
                return Math.min(value + delta * length, limit);
            }
