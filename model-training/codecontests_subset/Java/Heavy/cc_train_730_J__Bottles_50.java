...
public class CodeJ {
  ...
  static class PartialSolution {
    ...
    void expandTo(int max) {
      if (solutions.length < max + 1) {
        solutions = Arrays.copyOf(solutions, max + 1);
      }
    }
  }
  ...
  public static void main() {
    ...
    int totalVolume = 0;
    for (int i = 0; i < n; i++) {
      bottles[i].remaining = sc.nextInt();
      totalVolume += bottles[i].remaining;
    }
    for (int i = 0; i < n; i++) {
      bottles[i].volume = sc.nextInt();
    }
    Bottle[] bottlesByVolume = bottles.clone();
    Arrays.sort(bottlesByVolume);
    int cV = 0;
    int usedBottles = -1;
    for (int i = 0; i < n; i++) {
      cV += bottlesByVolume[i].volume;
      if (cV >= totalVolume) {
        usedBottles = i + 1;
        break;
      }
    ...
    for (int i = 0; i < usedBottles; i++) {
      solutions.add(new PartialSolution());
    }
    int best = 0;
    for (int cB = bottlesByVolume.length - 1; cB >= 0; cB--) {
      final int currentVolume = bottlesByVolume[cB].volume;
      final int currentRemaining = bottlesByVolume[cB].remaining;
      for (int cS = solutions.size() - 1; cS >= 1; cS--) {
        final PartialSolution current = solutions.get(cS);
        if (current.solutions.length == 0) {
          continue;
        }
        final int[] currentValues = current.solutions;
        if (cS == solutions.size() - 1) {
          for (int i = currentValues.length - 1; i >= 1; i--) {
            int nextIndex = i + currentVolume;
            if (nextIndex >= totalVolume) {
              best = Math.max(best, currentValues[i] + currentRemaining);
            } else {
              break;
            }
          }
        ...
        final PartialSolution next = solutions.get(cS + 1);
        next.expandTo(current.solutions.length - 1 + currentVolume);
        final int[] nextValues = next.solutions;
        for (int i = currentValues.length - 1; i >= 1; i--) {
          final int nextIndex = i + currentVolume;
          nextValues[nextIndex] = Math.max(nextValues[nextIndex],
              currentValues[i] + currentRemaining);
        }
      }
      if (solutions.size() == 1) {
        if (currentVolume >= totalVolume) {
          best = Math.max(best, currentRemaining);
        }
        continue;
      }
      final PartialSolution first = solutions.get(1);
      first.expandTo(currentVolume);
      final int[] firstValues = first.solutions;
      firstValues[currentVolume] = Math.max(firstValues[currentVolume], currentRemaining);
    }
    sw.printLine(usedBottles + " " + (totalVolume - best));
  }
