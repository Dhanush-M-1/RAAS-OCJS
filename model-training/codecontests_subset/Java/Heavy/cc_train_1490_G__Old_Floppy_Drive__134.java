...
static  int bs( int s ,int e  ,long key)
{
    if( s> e)return 0 ;
...
long[] arr= new long[n] ;long[] pre= new long[n] ;
...
for(int i = 1;i < n ;i++){pre[i]= pre[i-1] + arr[i] ; if(pre[i] > lp)lp = pre[i]; }


ArrayList<Pair> l = new ArrayList<>() ;

l.add(new Pair(pre[0] , 0 ));
...
for(int i = 0 ; i < l.size() ; i++ )st[i] =l.get(i) ;

Arrays.sort(st , new Comparator<Pair>() {
			@Override
			public int compare(Pair a, Pair b) {
				if (a.first != b.first) {
				    ...
				    else return 1; // for increasing order of first
				}
				
				
				return -1; //if first is same then sort on second basis
			}
		});



int[] minindex = new int[l.size()];

minindex[l.size()-1] = st[l.size()-1].second  ;


for(int i = l.size()-2 ; i>= 0 ; i--)minindex[i] = Math.min(minindex[i+1],st[i].second);
...
while(m-- > 0)
{
    ...
    if(arr[0] >= x)out.print(0+" ") ;
    
    
    
    else if( x > lp)
    {
        if(pre[n-1] <= 0)out.print(-1+ " ") ;
            ...
            long ans = (temp+pre[n-1]-1)/pre[n-1];
            
            long k = ans  ;
            
            ans = ans*(n-1) ;
            
            // if(k*pre[n-1] >= x)out.print(ans+ " ") ;
            
            // else{
                
                long req  = x -(k*pre[n-1]) ;
                ...
                int c = bs(0 ,st.length-1 ,req) ;
     
     
                int tem = minindex[st.length-c] ;
        ...
        int c = bs(0 ,st.length-1 ,x) ;
     
     
        int temp = minindex[st.length-c] ;
