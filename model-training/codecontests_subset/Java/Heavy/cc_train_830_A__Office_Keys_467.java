...
import java.math.*;
    ...
    {     static int p,k,n;
          static long[] a;
          static long[] b;  
          public static long calculate(int i,int j)
          {
             return (Math.abs(b[j]-a[i])+Math.abs(p-b[j]));
          }
          public static long check()
          {
            long[][] dp=new long[1050][2050];
            ...
            for(int i=0;i<=n;i++)
            {
              for(int j=0;j<=k;j++){
                ...
                if (j != k && i != n)
                    dp[i + 1][j + 1] =(long) Math.min(dp[i + 1][j + 1], Math.max(dp[i][j], calculate(i, j)));
              }
            }
            return dp[n][k];
          }
          ...
          {    
               ...
               for(int i=1;i<=k;i++)b[i]=l();
               double ans=(1L<<63)-1;
               double low=0;double high=ans;
               ...
               Arrays.sort(b);
                  for (int i=1;i<=k-n+1;i++){
                      long tmp=0;
                        for (int j=1;j<=n;j++){
                              tmp=Math.max(tmp,0L+Math.abs(a[j]-b[j+i-1])+Math.abs(p-b[j+i-1]));
                            }
