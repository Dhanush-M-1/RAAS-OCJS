...
public class Main {
    ...
    static long f(long n, long k, long o) {
        long res = 0;
        //System.out.print(n + " " + k + " " + o + " ");
        int i = (int)k;
        if (i > 0) o--;
        while (n > 0) {
            long t = n % 10;
            n /= 10;
            res += (t * Math.pow(10, k)) % 998244353;
            res = res % 998244353;
            if (i % 2 != k) {
                o--;
            }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            dCnt[i] = dCnt(a[i]);
            cnt.put(dCnt[i], cnt.getOrDefault(dCnt[i],0) + 1);
        }
        //printer.println(cnt);
        for (int i = 0; i < n; i++) {
            for (Map.Entry<Integer,Integer>e:cnt.entrySet()) {
                ans += (f(a[i], 0, e.getKey()) * e.getValue()) % 998244353;
                ans += (f(a[i], 1, e.getKey()) * e.getValue()) % 998244353;
                ans = ans % 998244353;
            }
        }
        //printer.println(Arrays.toString(dCnt));
        printer.println(ans);
        //printer.println(f(7777,1,3));
        printer.flush();
        printer.close();
    }
}
