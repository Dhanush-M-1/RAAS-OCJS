...
//    SegTree tree;
 ...
 /*   static int prime_len=1000010;
   ...
   public void solve(int testNumber, FastScanner in, PrintWriter out) throws IOException {
	   ...
	   double lo=-1000000000;
	   double hi=1000000000;
	   double pdiff=hi-lo;
	   while(hi-lo>=0.00000000001 ){
		   double m1=lo+(hi-lo)/3;
		   double m2=hi-(hi-lo)/3;
		   if(givepoorness(m2)<givepoorness(m1)){
			   lo=m1;
		   }
		   else{
			   hi=m2;
	   ...
	   out.println(Math.min(givepoorness(lo),givepoorness(hi)));
   }
   double givepoorness(double x){
	   ...
	   ans=a[0]-x;
	   for(int i=1;i<n;i++){
		   max[i]=Math.max(max[i-1]+a[i]-x,a[i]-x);
		   ans=Math.max(ans, max[i]);
	   }
	   min[0]=a[0]-x;
	   ans=Math.max(ans, -min[0]);
	   for(int i=1;i<n;i++){
		   min[i]=Math.min(min[i-1]+a[i]-x, a[i]-x);
