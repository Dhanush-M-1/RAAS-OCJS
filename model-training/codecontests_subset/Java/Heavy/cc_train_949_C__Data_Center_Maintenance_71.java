...
public class Main {
    ...
    static class ProblemCDataCenterMaintenance {
        public void solve(int testNumber, InputReader in, OutputWriter out) {
            ...
            StronglyConnectedComponent scc = new Tarjan(g);
    ...
    static class Tarjan implements StronglyConnectedComponent {
        int[] dfn;
        int[] state;
        int[] low;
        int vertexNum;
        ...
        void dfs(Graph g, int u) {
            ...
            for (Edge e : g.getEdges(u)) {
                if (state[e.to] == 0) dfs(g, e.to);
                if (state[e.to] == 1) low[u] = Math.min(low[u], low[e.to]);
            }
            if (low[u] == dfn[u]) {
                while (true) {
                    int v = stack.remove();
        ...
        public int[][] getComponents() {
            ...
            for (int i = 0; i < vertexNum; i++) components[low[i]].add(i);
            for (int i = 0; i < scc; i++) ret[i] = components[i].toArray();
            return ret;
        }
