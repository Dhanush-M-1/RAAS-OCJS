...
public class Main{
    ...
    public static void main(String args[]){
        ...
        while(t-->0){
            ...
            for(i=0;i<m;i++){
                Long x=tm.ceilingKey(brr[i]);
                if(x!=null){
                    w.print(tm.get(x)+" ");
                }else if(pre[n-1]<=0){
                    w.print(-1+" ");
                }else{
                    long left,cycle,need,ans;
                    left=brr[i]-tm.lastKey();
                    cycle=(left+pre[n-1]-1)/pre[n-1];
                    need=brr[i]-cycle*pre[n-1];
                    ans=cycle*n+tm.get(tm.ceilingKey(need));
                    w.print(ans+" ");
                }
            }
            w.println();
