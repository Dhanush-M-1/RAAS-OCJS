	...
	void solve()
	{
		...
		for(int z = 0;z < Q;z++){
			...
			for(int i = m-1;i >= 0;i--){
				int ind = (int)ss[i];
				if(l <= ind && ind <= r){
					if(ds.union(fs[ind]*2, ts[ind]*2+1))continue;
					ds.union(fs[ind]*2+1, ts[ind]*2);
					if(ds.equiv(fs[ind]*2, fs[ind]*2+1)){
						out.println(ss[i]>>>32);
						continue outer;
					}
				}
...
//		}
//		for (int i = 0; i < n; i++) {
//			int v1 = gen.nextInt(m/2);
//			int v2 = gen.nextInt(m/2)+m/2;
