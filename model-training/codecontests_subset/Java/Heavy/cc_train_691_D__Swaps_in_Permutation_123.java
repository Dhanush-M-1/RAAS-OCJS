...
public class cf691d {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for(int i = 0; i < n; ++i) {
            comps.computeIfAbsent(find(i), k -> new ArrayList<>()).add(i);
        }
        for(Integer k : comps.keySet()) {
            Collections.sort(comps.get(k));
            sortt(p, comps.get(k));
        }
        prln(p);
    ...
    static void sortt(int[] a, List<Integer> inds) {
        int[] temp = new int[inds.size()];
        for(int i = 0; i < temp.length; ++i) {
            temp[i] = a[inds.get(i)];
        }
        sort(temp);
        for(int i = temp.length - 1; i >= 0; --i) {
            a[inds.get(i)] = temp[temp.length - i - 1];
        }
    }
