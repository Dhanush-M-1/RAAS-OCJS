...
public class Main {
    ...
    static class TaskD {
        ...
        public void solve(int testNumber, FastReader in, PrintWriter out) {
            ...
            dfs(0, -1);
            int[] ord = GraphUtils.sortTopologically(gg);
            out.println("YES");
    ...
    static class GraphUtils {
        public static int[] sortTopologically(List<Integer>[] g) {
            int n = g.length;
            int[] indeg = new int[n];
            ...
            //sources
            for (int i = 0; i < n; ++i)
                if (indeg[i] == 0) ret[ptr++] = i;

            //remaining nodes
            for (int start = 0; start < ptr; ++start)
                for (Integer next : g[ret[start]])
                    if (--indeg[next] == 0) ret[ptr++] = next;

            //loop
            for (int i = 0; i < n; ++i)
                if (indeg[i] > 0) return null;
            return ret;
        }
