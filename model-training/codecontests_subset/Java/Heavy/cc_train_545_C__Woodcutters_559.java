...
//		boolean flag = m == 200000;
//		List<Pair<IntPair, Integer>> events = new ArrayList<>(), events2 = new ArrayList<>();
//		int[] L = new int[n], R = new int[n], A = new int[m], B = new int[m], C = new int[m], max = new int[m], best = new int[m];
//		IOUtils.readIntArrays(in, L, R);
//		IOUtils.readIntArrays(in, A, B, C);
//		TreapSet<IntPair> set = new TreapSet<>();
//		Set<Integer> used = new TreapSet<>();
//		for (int i = 0; i < n; i++) {
//			events.add(Pair.makePair(new IntPair(L[i], 0), i));
//			events.add(Pair.makePair(new IntPair(R[i], 2), i));
//			events2.add(Pair.makePair(new IntPair(L[i], 1), i));
//			events2.add(Pair.makePair(new IntPair(R[i], 3), i));
//			used.add(L[i]);
//			used.add(R[i]);
//		}
//		for (int i = 0; i < m; i++) {
//			events.add(Pair.makePair(new IntPair(A[i], 1), i));
//			events.add(Pair.makePair(new IntPair(B[i], 3), i));
//			events2.add(Pair.makePair(new IntPair(A[i], 0), i));
//			events2.add(Pair.makePair(new IntPair(B[i], 2), i));
//			used.add(A[i]);
//			used.add(B[i]);
//		}
...
//		Map<Integer, Integer> map = new EHashMap<>();
//		for (int i : used)
//			map.put(i, idx++);
//		IntervalTree tree = new LongIntervalTree(used.size()) {
...
//			}
//		};
//		if (!flag) for (Pair<IntPair, Integer> pair : events) {
//			int x = pair.second;
//			if (pair.first.second == 0)
//				set.add(new IntPair(R[x], x));
//			else if (pair.first.second == 1) {
//				if (!set.isEmpty()) {
//					IntPair pairMax = set.last();
//					int length = Math.min(pairMax.first, B[x]) - A[x];
//					if (length > max[x]) {
//						max[x] = length;
//						best[x] = pairMax.second;
//					}
//				}
...
////				if (x == 4620)
////					out.printLine(aux, length, y, A[x], map.get(A[x]), B[x],
////							map.get(B[x]));
//				if (length > max[x]) {
//					max[x] = length;
//					best[x] = y;
//				}
//			} else {
//				tree.update(map.get(L[x]), map.get(L[x]), 1L * (R[x] - L[x])
//						* n + x);
////				if (tree.query(0, used.size()-1)!=1L*(R[x]-L[x])*n+x) out.printLine(x, L[x], R[x], tree.query(0, used.size()-1));
//			}
//		}
//		set.clear();
//		if (!flag) for (Pair<IntPair, Integer> pair : events2) {
//			int x = pair.second;
//			if (pair.first.second == 3)
//				set.add(new IntPair(L[x], x));
//			else if (pair.first.second == 2) {
//				if (!set.isEmpty()) {
//					IntPair pairMin = set.first();
//					int length = B[x] - Math.max(A[x], pairMin.first);
//					if (length > max[x]) {
//						max[x] = length;
//						best[x] = pairMin.second;
//					}
//				}
//			}
//		}
...
////		if (m == 10000)
////			out.printLine(L[78], R[78], A[4620], B[4620], C[4620]);
//	}
	...
	public void solve(int testNumber, InputReader in, OutputWriter out) {
		...
		for (int i=1; i<n; i++) {
			dp[i][0]=dp[i-1][0];
			if (h[i-1]<x[i]-x[i-1]) dp[i][0]=Math.max(dp[i][0], dp[i-1][1]);
			if (h[i]<x[i]-x[i-1]) dp[i][0]=Math.max(dp[i][0], dp[i-1][0]+1);
			if (h[i]+h[i-1]<x[i]-x[i-1]) dp[i][0]=Math.max(dp[i][0], dp[i-1][1]+1);
			dp[i][1]=dp[i-1][0]+1;
			if (h[i-1]<x[i]-x[i-1]) dp[i][1]=Math.max(dp[i][1], dp[i-1][1]+1);
		}
		out.printLine(dp[n-1][1]);
	}
}
