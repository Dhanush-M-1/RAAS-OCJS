...
public class CompleteTheGraph {
    ...
    public static void main(String[] args) {
        ...
        long[][] allDists = new long[N][N];
        for (int i = 0; i < N; i++) {
        	allDists[i] = calcDist(nodes[i], prev, N);
        }
        
        setBlanks(edges, 1);
        
        long[] dist = calcDist(nodes[S], prev, N);
        ...
        while (dist[T] < L && path.length > 0) {
        	path[0].weight += L - dist[T];
        	dist = calcDist(nodes[S], prev, N);
        	path = getBlanksOnPath(nodes[T], prev, nodes);
        }
    ...
    public static long forceRange(long val, long lo, long hi) {
    ...
    public static long[] calcDist(Node start, Node[] prev, int N) {
