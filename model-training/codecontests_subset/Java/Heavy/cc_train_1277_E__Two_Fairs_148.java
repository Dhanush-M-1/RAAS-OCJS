...
{    
  ...
  public Main() {
    ...
    {        
       ...
       for (int i = 1; i <= n; i++) {
//System.out.println("root = " + s + " size of " + i + " = " + subTreeSize[i]);
           subTreeSize[i] = 0;
           visited[i] = false;
       }
    ...
    //return size of subtree containing special
    private long dfs(int root, int par, int chosenRoot, int special, int[][] nbs, int[] size, boolean[] visited)
    {
       int sz = 1;
       visited[root] = true;
       if (nbs[root].length == 1 && par > 0) {
           //leaf node
           size[root] = 1;
