...
public class CompleteTheGraph {
    ...
    public static void calcDist(Node start, int N, long[] dists, Node[] prev) {
    	Arrays.fill(dists, MAX);
    	TreeSet<PQElt> ts = new TreeSet<PQElt>();
...
//    					pq.offer(next);
    ...
    public static class PQElt implements Comparable<PQElt> {
    	...
    	public int compareTo(PQElt pqe) {
    		int dd = Long.compare(this.dist, pqe.dist);
    		if (dd != 0) {
    			return dd;
    		} else {
    			return Integer.compare(this.trgt.index, pqe.trgt.index);
    		}
    	}
    }
