...
public class Main {
    ...
    static class TaskD {
        ...
        public void solve(int testNumber, MyBufferedReader in, PrintWriter out) {
            ...
            TaskD.DjikResult djik = djik(s, ns, weights);
            long length = djik.dist[t];
            if (length < L) {
                boolean[][] isOnShortestPath = new boolean[n][n];
                int prev = djik.parents[t];
                int curr = t;
                while (prev != -1) {
                    isOnShortestPath[prev][curr] = isOnShortestPath[curr][prev] = true;
                    curr = prev;
                    prev = djik.parents[prev];
                }

                for (ArrayList<Integer> edge : all) {
                    int g1 = edge.get(0);
                    int g2 = edge.get(1);

                    if (!isOnShortestPath[g1][g2] && isBlank[g1][g2]) {
                        weights[g1][g2] = weights[g2][g1] = L + 1;
                    }
                }

                int[] edgeOnPath;
                do {
                    djik = djik(s, ns, weights);
                    long[] ds = djik.dist;
                    length = ds[t];
                    if (length >= L) {
                        break;
                    ...
                    edgeOnPath = getUntouchedBlank(t, djik.parents, isBlank, weights);
                    if (edgeOnPath == null) {
                        break;
                    }
        ...
        private int[] getUntouchedBlank(int curr, int[] parents, boolean[][] isBlank, long[][]
                weights) {
            int prev = parents[curr];
            while (prev != -1 && !(isBlank[prev][curr] && weights[prev][curr] == 1)) {
                curr = prev;
                prev = parents[curr];
            }
        ...
        static TaskD.DjikResult djik(int source, ArrayList<ArrayList<Integer>> neighbours,
                                     long[][] weights) {
