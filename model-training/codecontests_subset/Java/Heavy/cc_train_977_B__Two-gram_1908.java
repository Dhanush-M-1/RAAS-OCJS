...
public class Main {
  ...
  public static class Multiset<E> {
    HashMap<E,Integer> tree = new HashMap<E,Integer>();
    Integer add(E obj) {
      return tree.merge(obj,1,Integer::sum);
    }
    Integer remove(E obj) {
      return tree.merge(obj,-1,(a,b)->a+b == 0 ? null : a+b);
    }
    Integer get(E obj) {
      return tree.getOrDefault(obj, 0);
    }
  ...
  public static void main() {
    ...
    String maxS = "";
    int max = 0;
    for (Map.Entry<String, Integer> e : set.tree.entrySet()) {
      if (e.getValue() > max) {
        max = e.getValue();
        maxS = e.getKey();
      }
    }
    outWriter.println(maxS);
  }
