    ...
    private void solveA() throws IOException {
        ...
        for (long i : a) {
            ...
            for (int j = 0; cur > 0; cur /= 10, j++) {
                ...
                ans += ten[j * 2] * (prefCnt[prefCnt.length - 1] - prefCnt[max(0, j - 1)]) % mod * digit % mod;
                ans %= mod;
                for (int k = 1; j + k < j * 2; k++) {
                    ans += ten[j + k] * (prefCnt[k] - prefCnt[k - 1]) % mod * digit % mod;
                    ans %= mod;
                }

                ans += ten[j * 2 + 1] * (prefCnt[prefCnt.length - 1] - prefCnt[j]) % mod * digit % mod;
                ans %= mod;
                for (int k = 1; j + k <= j * 2; k++) {
                    ans += ten[j + k] * (prefCnt[k] - prefCnt[k - 1]) % mod * digit % mod;
                    ans %= mod;
