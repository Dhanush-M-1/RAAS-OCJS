	...
	void solve() throws IOException {
		...
		for (int year = 2013; year <= 2015; ++year) {
			...
			for (int month = 1; month <= 12; ++month) {
				...
				for (int day = 1, maxDay = days[month - 1]; day <= maxDay; ++day) {
					String dayString = (day < 10? "0": "") + day;
					
					String dateString = dayString + monthYearString;
					
					int count = getCount(message, dateString);
					if (count > maxCount) {
						maxCount = count;
		...
		out.println(maxDateString);
	}
	
	final int[] days = {31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
	
	int getCount(String message, String dateString) {
		int count = 0;
		
		int dateLength = dateString.length();
		int length = message.length();
		
		for (int startIndex = 0, index = message.indexOf(dateString, startIndex); 
				startIndex < length && index >= 0;
				startIndex = index + dateLength, index = message.indexOf(dateString, startIndex), ++count);
