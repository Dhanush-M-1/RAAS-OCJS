...
public class Solution {
    ...
    private static boolean check(int[] pl, int[] kl, long o, long m) {
        int i1 = 0;
        int i2 = 0;

        while (i1 < pl.length) {
            if (i2 == kl.length) return false;
            long km = Math.abs(pl[i1] - kl[i2]) + Math.abs(o - kl[i2]);
            i2++;
            if (km <= m) {
                i1++;
            }
