...
public class Main {
    ...
    static class TaskD {
        public void solve(int testNumber, FastScanner in, FastPrinter out) {
            Expression[] expressions = new ExpressionBuilder(in.nextDoubleArray(3)).build();
            Arrays.sort(expressions);
            out.print(expressions[expressions.length - 1]);
        }
    ...
    static class ExpressionBuilder {
        ...
        public Expression[] build() {
            Expression[] expressions = new Expression[12];
            int count = 0;
            for (int i = 0; i < 3; i++) {
                for (int type = 0; type < 2; type++) {
                    for (int j = 0; j < 3; j++) {
                        if (i == j) continue;
                        expressions[count] = new Expression(values[i], values[j], values[3 - i - j], type, count++,
