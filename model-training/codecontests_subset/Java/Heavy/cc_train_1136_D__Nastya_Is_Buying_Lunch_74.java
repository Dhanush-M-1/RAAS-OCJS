...
public class D {
    ...
    static class Solver {
        ...
        void solve() {
            ...
            for (int i=0; i<n; i++) {
                initialOrder[i] = io.nextInt();
            }
            int nastya = initialOrder[n-1];

            boolean[] givesToNastya = new boolean[n+1];
            List<Integer>[] take = new ArrayList[n+1];
            for (int i=1; i<=n; i++) take[i] = new ArrayList<>();
            for (int i=0; i<m; i++) {
                int u = io.nextInt();
                int v = io.nextInt();
                if (v == nastya) givesToNastya[u] = true;
                take[v].add(u);
            }

            HashSet<Integer> passable = null;

            int ans = 0;
            for (int i=n-2; i>=0; i--) {
                int node = initialOrder[i];
                if (givesToNastya[node]) {
                    if (passable == null || passable.contains(node)) {
                        ans++;
                        continue;
                    }
                }
                HashSet<Integer> constrainedPassable = new HashSet<>();
                for (int allowPass : take[node]) {
                    if (passable == null || passable.contains(allowPass)) {
                        constrainedPassable.add(allowPass);
                    }
                }
                passable = constrainedPassable;
            }
