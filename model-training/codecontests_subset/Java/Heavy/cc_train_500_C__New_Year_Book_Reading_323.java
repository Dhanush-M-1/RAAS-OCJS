...
class TaskC {
    public void solve(int testNumber, InputReader in, OutputWriter out) {
        ...
        //for (int i = 0; i < addOrder.length; i++) curstate.add(addOrder[i].second);
//        curstate.add(1);
//        curstate.add(3);
//        curstate.add(2);
        for (int vall : b) if (!curstate.contains(vall)) curstate.add(vall);
        //for(int val : curstate) out.print(val, "  ");
        for (int i = 0; i < days; i++) {
            int indexofbooktoberead = curstate.indexOf(b[i]);

            //out.printLine(b[i],indexofbooktoberead);

            for (int j = 0; j < indexofbooktoberead; j++) lifted[curstate.get(j)-1]++;
            int remval = b[i];
            int useless = curstate.remove(indexofbooktoberead);
            curstate.add(0, remval);
//            out.print(b[i], indexofbooktoberead, "   " );
//            for(int val : curstate) out.print(val + " ");
//            out.printLine();
        }
//        out.printLine(lifted);
//        out.printLine(w);
        //Arrays.sort(lifted);
       // Arrays.sort(w);
