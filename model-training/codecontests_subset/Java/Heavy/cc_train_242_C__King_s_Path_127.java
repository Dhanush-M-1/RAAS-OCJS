...
public class C implements Runnable{
    ...
    private void solve() {
        ...
        Set<Point> goodPoints = new HashSet<>();
        ...
        while (n --> 0) {
            ...
            for (int column = left; column <= right; ++column) {
                goodPoints.add(new Point(row, column));
            }
        }

        int distance = getDistance(start, end, goodPoints);
        out.println(distance);
    }

    private static final int NO_WAY = -1;

    private static int getDistance(Point start, Point end, Set<Point> goodPoints) {
        ...
        while (queue.size() > 0) {
            ...
            int fromDistance = distances.get(from);
            ...
            for (int[] step : steps8) {
                ...
                if (!goodPoints.contains(to)) continue;
                if (distances.containsKey(to)) continue;

                distances.put(to, fromDistance + 1);
                queue.add(to);
            }
        }

        return distances.getOrDefault(end, NO_WAY);
    }
