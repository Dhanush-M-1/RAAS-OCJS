    ...
    */
    ...
    public static long mod = 1000000000+7;
        ...
        public static void main(String[] args) throws java.lang.Exception{
            //let_me_start 
         int n = i(); int m=i();
         int arr[] = new int[n+1];// is(n);
         for(int i=1;i<=n;i++)arr[i]=i();
         int f[] = new int[n+1];
         int mm[] = is(m);
        // for(int i=1;i<=m;i++)f[mm[i]]++;
          //String ans = "NO";
         long ans=0;
         Pair[] pair = new Pair[n+1];
         int t=1;
         for(int i=1;i<=m;i++){
          if(f[mm[i]]==1)continue;
          pair[t]=new Pair(mm[i],arr[mm[i]],1);
          f[mm[i]]=1;
          t++;
         }
         for(int i=1;i<=n;i++){
         if(f[i]==1)continue;
         pair[t]=new Pair(mm[i],arr[i],1);
         f[i]=1;
         t++;
         
         }
        /*for(int i=1;i<=n;i++){
         // pair[i]=new Pair(arr[i],f[i]);
         
         out.write("ar"+arr[i]+"a=="+pair[i].a+" b=="+pair[i].b+" c=="+pair[i].c+"\n");
         }
    */  //Arrays.sort(pair,1,n+1);
        
         ans=0;
         int ch=0,idx=0;Pair p=null ;
        for(int i=1;i<=m;i++){
         for(int j=1;j<=n;j++){
          
          if(mm[i]==pair[j].c){
            
            p= new Pair(pair[j].c,pair[j].a,pair[j].b);
            idx=j;
            break;
          }else{
        ...
        //long ans=0;
        //for(int i=1;i<=n;i++)out.write(""+sum[i]+" ");;
        //for(int i=1;i<=n;i++)ans+=sum[i]*pair[i].a;
        out.write(""+ans+"\n");
        out.flush();
        return;
        }    
