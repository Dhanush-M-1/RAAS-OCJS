...
public class C implements Runnable{
	...
	void solve() {
		...
		IncrementSegmentTree bTree = new IncrementLimitedSegmentTree(n, b);
		IncrementSegmentTree aTree = new IncrementLimitedSegmentTree(n, a);
		...
		while (q --> 0) {
			...
			if (type == 1) {
				...
				if (start > 0) {
					sum += bTree.getSum(0, start - 1);
				}
				
				if (end < n - 1) {
					sum += aTree.getSum(end + 1, n - 1);
				}
	...
	interface IncrementSegmentTree {
		void inc(int index, int value);
		long getSum(int left, int right);
	}
	
	class IncrementLimitedSegmentTree implements IncrementSegmentTree {
		...
		IncrementLimitedSegmentTree(int size, int limit) {
			this.size = size;
		...
		public void inc(int index, int value) {
			inc(1, 0, size - 1, index, value);	
		}
		
		void inc(int v, int curLeft, int curRight, int index, int value) {
			if (curLeft == curRight) {
				tree[v] = min(tree[v] + value, limit);
			} else {
				int curMid = ((curLeft + curRight) >> 1);
				
				int vLeft = (v << 1);
				int vRight = (vLeft + 1);
				
				if (index <= curMid) {
					inc(vLeft, curLeft, curMid, index, value);
				} else {
					inc(vRight, curMid + 1, curRight, index, value);
				}
		...
		long get(int v, int curLeft, int curRight, int left, int right) {
			...
			if (curLeft == left && right == curRight) {
				...
				long leftSum = get(vLeft, curLeft, curMid, left, min(curMid, right));
				long rightSum = get(vRight, curMid + 1, curRight, max(left, curMid + 1), right);
