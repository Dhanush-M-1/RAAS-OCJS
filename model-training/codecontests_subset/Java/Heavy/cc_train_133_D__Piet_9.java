...
public class D {
    ...
    public void solve() throws Exception {
        ...
        int rows = nextInt();
        int n = nextInt();
        String[] stmp = nextArrS(rows);
        int cols = stmp[0].length();
        int[][] p = new int[rows+2][cols+2];
        for (int[] i: p) Arrays.fill(i,-1);
        for (int i=1; i<=rows; ++i) for (int j=1; j<=cols; ++j) p[i][j] = stmp[i-1].charAt(j-1)-48;

        int[][] blockID = new int[rows+2][cols+2];
        int[][] blockEnd = new int[3000][4];
        int id = 0;
        for (int i=1; i<=rows; ++i) for (int j=1; j<=cols; ++j) if (p[i][j]>0) {
            if (p[i][j]==p[i-1][j] || p[i][j]==p[i][j-1]) {
                blockID[i][j] = p[i][j]==p[i-1][j] ? blockID[i-1][j] : blockID[i][j-1];
                blockEnd[blockID[i][j]][1] = max(1+j,blockEnd[blockID[i][j]][1]);
                blockEnd[blockID[i][j]][2] = max(1+i,blockEnd[blockID[i][j]][2]);
            } else {
                blockID[i][j] = ++id;
                blockEnd[id][0] = i-1;
                blockEnd[id][3] = j-1;
        ...
        int block = 1, dir = 1, chos = 0;
        int steps = 0;
        HashMap<Integer,Integer> states = new HashMap<Integer,Integer>();
        
        while (true) {
            
            Integer x = states.get((chos<<14) | (dir<<12) | block);
            if (x!=null) n%=steps-x;
            states.put((chos<<14) | (dir<<12) | block, steps);
            

            if (n==0)
                for (int i=0; i<p.length; ++i) for (int j=0; j<p[i].length; ++j) if (blockID[i][j]==block) halt(p[i][j]);

            int xx=0, yy=0;
            if (dir==0 && chos==0) {xx=blockEnd[block][3]+1; yy=blockEnd[block][0];}
            if (dir==0 && chos==1) {xx=blockEnd[block][1]-1; yy=blockEnd[block][0];}
            if (dir==1 && chos==0) {xx=blockEnd[block][1]; yy=blockEnd[block][0]+1;}
            if (dir==1 && chos==1) {xx=blockEnd[block][1]; yy=blockEnd[block][2]-1;}
            if (dir==2 && chos==0) {xx=blockEnd[block][1]-1; yy=blockEnd[block][2];}
            if (dir==2 && chos==1) {xx=blockEnd[block][3]+1; yy=blockEnd[block][2];}
            if (dir==3 && chos==0) {xx=blockEnd[block][3]; yy=blockEnd[block][2]-1;}
            if (dir==3 && chos==1) {xx=blockEnd[block][3]; yy=blockEnd[block][0]+1;}
            if (blockID[yy][xx]>0) block = blockID[yy][xx];
            else
                if (chos==0) chos=1;
                else {chos=0; dir=(dir+1)%4;}
            steps++;
            n--;
        }
    }
