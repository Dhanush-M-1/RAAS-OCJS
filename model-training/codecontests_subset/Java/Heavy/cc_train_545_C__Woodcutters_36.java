...
class TaskC {
	...
	void goRecursive(int lastTreeEnds, int indexStartTree, int currentRes) {
		ans = Math.max(currentRes, ans);
		for (int i = indexStartTree; i < n; i++) {
			Pair<Integer, Integer> tree = trees[i];
			int pos = tree.first;
			int h = tree.second;
			int leftPos = getLeftPos(i, lastTreeEnds);
			int rightPos = getRightPos(i);
    ...
    public void solve(int testNumber, InputReader in, OutputWriter out) {
		...
		goRecursive(Integer.MIN_VALUE, 0, 0);
		out.print(ans);
    }
}
