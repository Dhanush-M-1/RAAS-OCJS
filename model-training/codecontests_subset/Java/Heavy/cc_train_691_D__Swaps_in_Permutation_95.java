...
public class Main {
    ...
    static class TaskD {
        public void solve(int testNumber, InputReader in, PrintWriter out) {
            ...
            Map<Integer, TreeSet<Integer>> compElems = new HashMap<>();
            for (int rt : comps) compElems.put(rt, new TreeSet<>());
            for (int i = 0; i < n; i++) {
                int rt = d.root(i);
            ...
            int[] res = new int[n];
            for (int rt : comps) {
                TreeSet<Integer> elems = compElems.get(rt);
                TreeSet<Integer> vals = new TreeSet<>();
                for (int i : elems) {
                    vals.add(-p[i]);
                }
                Iterator<Integer> ik = elems.iterator();
                Iterator<Integer> iv = vals.iterator();
                while (ik.hasNext()) {
                    res[ik.next()] = -iv.next();
                }
            }
