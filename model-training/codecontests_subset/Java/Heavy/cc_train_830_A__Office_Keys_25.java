...
public class Main {
	...
	private static boolean f(ArrayList<Long> p, TreeSet<Long> hm, long o, long time) {

		int i = 0;

		while (i < p.size() && p.get(i) <= o) {
			...
			long extra = (time - dis) / 2;
			long pp = p.get(i) - extra;

			if (hm.ceiling(pp) == null)
				return false;

			else {

				long ce = hm.ceiling(pp);
