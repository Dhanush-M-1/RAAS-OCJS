...
public class Main{
    ...
    public static void main(String[] args) throws IOException{
        ...
        Arrays.sort(one, 1, ix1);
        Arrays.sort(two, 1, ix2);
        Arrays.sort(three, 1, ix3);
        
        if(ix1 - 1 + Math.min(ix2 - 1, ix3 - 1) < k) System.out.println(-1);
        else if(ix1 - 1 == 0) {
        	long s = 0;
        	for(int i=1; i<=k; i++) s += two[i] + three[i];
        	System.out.println(s);
        }else {
        	PriorityQueue<Long> q = new PriorityQueue<Long>();
        	for(int i=1; i<ix1; i++) q.add(one[i]);
        	for(int i=1; i<Math.min(ix2, ix3); i++) q.add(two[i] + three[i]);
