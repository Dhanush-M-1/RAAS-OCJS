...
public class Main {
	...
	void solve(int T) {
		int n = ni();
		int k = ni();
		PriorityQueue<Integer> q[] = new PriorityQueue[4];
		for (int i = 0; i < 4; i++)
		...
		long ans = 0;
		while (k > 0) {
			if ((q[1].isEmpty() || q[2].isEmpty()) && q[3].isEmpty()) {
				out.print(-1);
				return;
			}
			if (q[1].isEmpty() || q[2].isEmpty())
				ans += q[3].poll();
			else if (q[3].isEmpty())
				ans += (q[1].poll() + q[2].poll());
			else if (q[3].peek() < q[1].peek() + q[2].peek())
				ans += q[3].poll();
			else
				ans += (q[1].poll() + q[2].poll());
			k--;
