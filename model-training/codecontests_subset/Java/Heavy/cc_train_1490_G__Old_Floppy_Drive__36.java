...
public class Main {
    ...
    static void testcase() throws IOException{
        int n=sc.nextInt(), m=sc.nextInt(), arr[]=sc.nextArr(n);
        long sum=0;
        for (int i = 0; i < n; i++)
            sum+=arr[i];
        Stack<LP> st=new Stack<>();
        sum=0;
        for (int i = 0; i < n; i++) {
            sum+=arr[i];
            LP temp=new LP(sum, i);
            if(st.isEmpty() || st.peek().x<sum)
                st.add(temp);
        }
        LP[] min=new LP[st.size()];
        for (int i = 0; i < min.length; i++)
            min[i]=st.pop();
        reverse(min);
//        for (int i = 0; i < min.length; i++)
//            pw.println(min[i].x+" "+min[i].y);
//        pw.flush();
        while(m-->0)
            solve(sum, min, sc.nextInt(), n);
        pw.println();
    }
    static void solve(long sum, LP[] min, int x, int len){
        int n=min.length;
        long need=x-min[n-1].x;
        long ans=0;
        if(need>0){
            if(sum<=0){
                pw.print("-1 ");
    ...
    static int test(int x, HashMap<Integer, Integer> map){
        int ans=0;
        for (Map.Entry<Integer, Integer> a: map.entrySet())
            ans+=a.getValue() >= x? a.getValue()-x: a.getValue();
        return ans;
    }
