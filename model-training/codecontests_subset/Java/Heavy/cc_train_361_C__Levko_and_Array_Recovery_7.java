...
//        int[] arr = s.nextIntArray(n);
        ...
        for (int i = 0; i < m; i++) {
            queries[i] = new pair(s.nextInt(), s.nextInt() - 1, s.nextInt() - 1, s.nextInt());
        }
        Arrays.fill(ans, Long.MAX_VALUE);

        for (int i = 0; i < queries.length; i++) {
            if (queries[i].num == 1) {
                for (int j = queries[i].from; j <= queries[i].to; j++) {
                    arr[j] += (long)queries[i].diffOrMax;
                }
            } else {
                for (int j = queries[i].from; j <= queries[i].to; j++) {
                    ans[j] = Math.min(ans[j], (long)queries[i].diffOrMax - arr[j]);
                }
            }
        }
...
//                long add = queri
        ...
        if(bool){
            ...
            out.println("YES");
            StringBuilder ansF = new StringBuilder();
...
//        out.println(Arrays.toString(ans));
    ...
    private static class pair{
        ...
        public pair(int num, int from, int to, int diffOrMax) {
            this.num = num;
            this.from = from;
            this.to = to;
            this.diffOrMax = diffOrMax;
        }
