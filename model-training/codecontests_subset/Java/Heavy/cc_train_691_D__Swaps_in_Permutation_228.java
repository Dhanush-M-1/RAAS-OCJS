...
public class CF_691D{
    //SOLUTION BEGIN
    void pre() throws Exception{}
    void solve(int TC) throws Exception{
        int N = ni(), M = ni();
        int[] P = new int[N], set = new int[N];
        for(int i = 0; i< N; i++){
            P[i] = ni();
            set[i] = i;
        }
        ...
        int[] ans = new int[N];
        for(int i = 0; i< N; i++){
            if(find(set, i) != i)continue;
            ArrayList<Integer> tmp = new ArrayList<>(list[i]);
            Collections.sort(tmp, (Integer i1, Integer i2) -> Integer.compare(P[i2], P[i1]));
            for(int j = 0; j< list[i].size(); j++)ans[list[i].get(j)] = P[tmp.get(j)];
        }
    ...
    public static void main(String[] args) throws Exception{
        if(memory)new Thread(null, new Runnable() {public void run(){try{new CF_691D().run();}catch(Exception e){e.printStackTrace();}}}, "1", 1 << 28).start();
        else new CF_691D().run();
    }
