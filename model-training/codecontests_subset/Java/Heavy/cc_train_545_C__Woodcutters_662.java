...
public class Contest {
    ...
    static void solve() throws IOException {
        int n = nextInt(), count=0, last=Integer.MIN_VALUE;
        Pair[] trees = new Pair[n];

        for (int i = 0; i <n ; i++) {
            trees[i] = new Pair(nextInt(), nextInt());
        }

        for (int i = 0; i <n ; i++) {
            if(i==n-1){
                count++;
            }else if (trees[i].tree-trees[i].height>last){
                count++;
                last = trees[i].tree;
            }else if (trees[i].tree+trees[i].height<trees[i+1].tree){
                count++;
                last = trees[i].tree+trees[i].height;
            }else last = trees[i].tree;
        }
        out.println(count);
    }
}
