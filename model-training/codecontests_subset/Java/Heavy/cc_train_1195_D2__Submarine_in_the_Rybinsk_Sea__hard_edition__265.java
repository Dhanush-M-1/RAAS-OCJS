...
public class cf1195d2 {

    public static void main(String[] args) throws IOException {
        int n = ri(), a[] = ria(n), cnt_len[] = new int[11], suf_cnt_len[] = new int[12];
        for (int i : a) {
            ++cnt_len[len(i)];
        }
        // suf_cnt_len[i] = #(len >= i)
        for (int i = 10; i > 0; --i) {
            suf_cnt_len[i] = suf_cnt_len[i + 1] + cnt_len[i];
        }
        int pow10[] = new int[20], ans = 0;
        pow10[0] = 1;
        for (int i = 1; i <= 19; ++i) {
            pow10[i] = mmul(10, pow10[i - 1]);
        }
        for (int i : a) {
            int x = 0;
            while (i > 0) {
                ...
                i /= 10;
                for (int l = 1; l <= x; ++l) {
                    sum = madd(sum, mmul(pow10[l + x], d, cnt_len[l]));
                }
                sum = mmul(2, sum);
                sum = madd(sum, mmul(11, pow10[2 * x], d, suf_cnt_len[x + 1]));
                ans = madd(ans, sum);
                ++x;
            }
        }
