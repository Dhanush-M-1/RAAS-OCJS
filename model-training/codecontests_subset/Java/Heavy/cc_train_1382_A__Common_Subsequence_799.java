...
public final class Code1 {
    ...
    public static void func(int n , int m, int[] a , int[] b){
        ...
        int c=-1;
        for (int i = 0; i < n; i++) {
            if(hs.contains(a[i])){c=a[i];break;}
        }
        
        if(c>0){prln("YES");prln(1+" "+c);}
        else prln("NO");
    }
    ...
    static void dbghs(Set<Integer> a) {prln(a.toString());}
    static void dbgral(List<Integer> al) {prln(al.toString());}
    ...
    // math util
    static int sum(int[] a){int sum=0;for(int i:a)sum=(sum + mod + i) % mod; return sum;}
    static long sum(long[] a){long sum=0;for(long i:a)sum=(sum + mod + i) % mod; return sum;}
    ...
    static int[][] nxtira(int r, int c) throws NumberFormatException, IOException {return rd.nextInt2dArray(r,c);}
    static long[] nxtlra() throws NumberFormatException, IOException {return rd.nextLongArray();}
    // static long[] nxtlra(int size){return rd.nextLongArray(size);}
    // static long[][] nxtlra(int r, int c){return rd.nextLong2DArray(r,c);}
    // static char[] nxtcra(){return rd.next().toCharArray();}
    // static String[] nxtsra(int size){return rd.nextStringArray(size);}
    ...
    static void prln(int[][] a){rd.println(a);}
    // static void pr(String[] a){rd.print(a);}
    // static void prln(String[] a){rd.println(a);}
    // // static <T> void prln(Collection<T> c) {int n = c.size() - 1; Iterator<T> iter = c.iterator(); for(int i = 0; i < n; rd.print(iter.next()), rd.print(' '), ++i); rd.println(iter.next());}
...
class FastIO1{
    ...
    public static int[][] nextInt2dArray(int r, int c) throws NumberFormatException, IOException {
        int[][] a = new int[r][c];
        for (int i = 0; i < r; i++) {
            String[] s = next().split(" ");
            for (int j = 0; j < c; j++) {
