...
public class Main {
    ...
    public static void main (String[] args) throws java.lang.Exception {
        ...
        while(test-->0) {
        	...
        	long l=0,r=n,group=-1;
        	while(l<=r) {
        		long mid=(long)(l+(r-l)/2L);
        		if(k<=(mid*(mid+1))/2L && k>(mid*(mid-1))/2L) {
        			group=mid;
        			break;
        		}
        		else if(k>(mid*(mid+1))/2L) l=mid+1;
        		else r=mid-1;
        	}
        	long idx=k-(group*(group-1))/2-1;
        	out.println(a[(int)idx]);
        }
        out.flush();
        out.close();
    }
