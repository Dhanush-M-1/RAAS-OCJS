...
public class cf1374e1 {
    ...
    public static void main(String[] args) throws IOException {
        int n = rni(), k = ni(), nboth = 0, na = 0, nb = 0, cnta = 0, cntb = 0;
        List<Integer> both = new ArrayList<>(), a = new ArrayList<>(), b = new ArrayList<>();
        for(int i = 0; i < n; ++i) {
            int t = rni(), ai = ni(), bi = ni();
            cnta += ai;
            cntb += bi;
            if(ai == 1 && bi == 1) {
                both.add(t);
        ...
        if(cnta < k || cntb < k) {
            ...
            Collections.sort(a);
            Collections.sort(b);
            int pboth = 0, pa = 0, pb = 0;
            for(int i = 0; i < k; ++i) {
                if(pboth < nboth && (pa == na || pb == nb || both.get(pboth) <= a.get(pa) + b.get(pb))) {
                    ans += both.get(pboth++);
                } else {
                    ans += a.get(pa++) + b.get(pb++);
                }
            }
            prln(ans);
        }
