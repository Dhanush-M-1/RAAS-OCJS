...
public class Main {
    ...
    static class TaskD {
        public void solve(int testNumber, InputReader in, OutputWriter out) {
            ...
            s = in.readInt();
            t = in.readInt();
            gr = new BidirectionalGraph(n, m);
            rib0 = new IntArrayList();
            for (int iter = 0; iter < m; iter++) {
                int a = in.readInt();
                int b = in.readInt();
                int w = in.readInt();
                if (w == 0) {
                    int id = gr.addWeightedEdge(a, b, 1);
                    rib0.add(id);
                } else {
                    gr.addWeightedEdge(a, b, w);
                }
            }


            {
                Pair<Long, IntList> res = ShortestDistance.dijkstraAlgorithm(gr, s, t);
                for (int j = 0; j < rib0.size(); j++) {
                    int id = rib0.get(j);
                    gr.removeEdge(id);
                    gr.removeEdge(gr.transposed(id));
                }
                Pair<Long, IntList> resRemove = ShortestDistance.dijkstraAlgorithm(gr, s, t);
            ...
            while (lc < rc) {
                ...
                Pair<long[], int[]> res = ShortestDistance.dijkstraAlgorithm(gr, s);
                if (L < res.first[t]) {
                    lc = mc + 1;
                } else {
                    rc = mc;
            ...
            if (lc != 0) {
                int idRib = rib0.get(lc - 1);
                long lVal = 1, rVal = Integer.MAX_VALUE;
                while (true) {
                    long mVal = (rVal + lVal) / 2;
                    gr.weight[idRib] = gr.weight[gr.transposed(idRib)] = mVal;
                    Pair<long[], int[]> res = ShortestDistance.dijkstraAlgorithm(gr, s);
                    if (res.first[t] < L) {
                        lVal = mVal + 1;
