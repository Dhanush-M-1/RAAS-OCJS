...
public class Main {
    ...
    static void solve() {
        ...
        for (int i = 0; i < n; i++) {
            data[i] = in.next().chars().map(x -> Character.getNumericValue(x)).toArray();
        }

        String res = res(data[0][1], data[1][0], data[n - 1][n - 2], data[n - 2][n - 1], data);
        // 01
        // 10
        // n-1 n-2
        // n-2 n-1
    ...
    static String res(int a, int b, int c, int d, int[][] data) {
