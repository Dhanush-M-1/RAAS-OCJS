...
public class D implements Runnable{
    ...
    private void solve() {
        ...
        int[][] edges = readIntMatrixWithDecrease(m, 2);
        ...
        List<Integer>[] componentIndexes = new List[n];
        List<Integer>[] componentValues = new List[n];

        for (int i = 0; i < n; ++i) {
            if (dsu.get(i) == i) {
                componentIndexes[i] = new ArrayList<>(0);
                componentValues[i] = new ArrayList<>(0);
            }
        }
        ...
        for (int i = 0; i < n; ++i) {
            ...
            componentIndexes[root].add(i);
            componentValues[root].add(value);
        }

        for (int i = 0; i < n; ++i) {
            if (dsu.get(i) == i) {
                Collections.sort(componentValues[i]);
                Collections.reverse(componentValues[i]);

                for (int j = 0; j < componentIndexes[i].size(); ++j) {
                    int index = componentIndexes[i].get(j);
                    int value = componentValues[i].get(j);
