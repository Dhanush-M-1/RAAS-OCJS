...
public class Main {
	...
	public static void main(String args[]) throws IOException {
		...
		int n=sc.nextInt();
		int k=sc.nextInt();
		PriorityQueue<BBOOK> both=new PriorityQueue<>();
		PriorityQueue<BBOOK> alice=new PriorityQueue<>();
		PriorityQueue<BBOOK> bob=new PriorityQueue<>();
		...
		int b=sc.nextInt();
		if(a==b&&a==1) {
			both.add(new BBOOK(t,a,b));
		}
		else if(a==1) {
			alice.add(new BBOOK(t,a,b));
		}
		else if(b==1) {
			bob.add(new BBOOK(t,a,b));
			}
		}
		int a=0;
		int b=0;
		...
		if(!both.isEmpty()&&!alice.isEmpty()&&!bob.isEmpty()) {
			int time1=both.peek().t;
			int time2=alice.peek().t+bob.peek().t;
			if(time1>time2) {
				alice.poll();
				bob.poll();
				a++;
