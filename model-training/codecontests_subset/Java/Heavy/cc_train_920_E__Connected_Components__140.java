...
//        MyReader reader = new MyReader(new FileInputStream("input.txt"));
    ...
    private void run(MyReader reader, MyWriter writer) throws Exception {
        ...
        if (n < 5000) {
            ...
            int q = 0;
            for (int i = 1; i < n; i++) {
                if (edges[i].size() < edges[q].size()) {
                    q = i;
                }
            }
            vis.add(q);
            for (int i = 0; i < n; i++) {
                if (edges[q].contains(i)) {
                    nvis.add(i);
                }
            }
            for (int i = 0; i < n; i++) {
    ...
    private Set<Integer> nvis = new HashSet<>();

    private void dfs(int n, Set<Integer>[] edges) {
        vis.add(n);
        for (int i = 0; i < edges.length; i++) {
            if (!vis.contains(i) && !edges[n].contains(i)) {
                dfs(i, edges);
            }
        }
    }

    private void dfs2(int n, Set<Integer>[] edges) {
        vis.add(n);
        for (int q : nvis) {
            if (!vis.contains(q) && !edges[n].contains(q)) {
                dfs2(q, edges);
            }
        }
    }
