...
public class Main{
       ...
       static  long val[][]=new long[3000001][2];
         ...
         public static void main(String[] args) 
        {
            ...
            for(int i=1;i<n-1;i++) {
            	...
            	if(val[i][1]<p[i+1].x)
            		dp[i][1]=Math.max(dp[i-1][1]+1, dp[i-1][0]+1);
            }
            
            if(n==1)
            	pw.println(1);
            else
            	pw.println(Math.max(dp[n-2][0],dp[n-2][1])+1);
            pw.flush();
            pw.close();
