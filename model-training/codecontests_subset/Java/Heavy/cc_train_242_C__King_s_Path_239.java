...
public class Main{
    ...
    static int bfs(Long s,Long d,HashMap<Long,ChessBoardSquare> board)
    {
        if(board.containsKey(d-1))
        {
            ...
            board.get(s - 1).color = 1;
            board.get(s - 1).distance = 0;
            board.get(s - 1).pred = -1l;
            q.add(s - 1);
            while (q.size() != 0)
            {
                ...
                for (int i = 0; i < 8; i++)
                {
                    ...
                    if (board.containsKey(adjpos) && !visited.contains(adjpos) && adjx >= 0 && adjx < 1000000000 && adjy >= 0 && adjy < 1000000000)
                    {
