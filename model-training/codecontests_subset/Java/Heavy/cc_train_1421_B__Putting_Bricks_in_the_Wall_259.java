    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < count; i++){
            ...
            for (int j = 0; j < size; j++){
                base[j] = readArrayString("");
            }
            int answer = 0;
            StringBuilder strAnswer = new StringBuilder();
            for (int j = 0; j <= 1; j++){
                if (base[0][1].equals(j == 0 ? "1" : "0")){
                    answer++;
                    strAnswer.append("1 2");
                }
                if (base[1][0].equals(j == 0 ? "1" : "0")){
                    if (answer > 0){
                        strAnswer.append("\n");
                    }
                    answer++;
                    strAnswer.append("2 1");
                }
                if (base[size - 2][size - 1].equals(j== 0 ? "0" : "1")){
                    if (answer > 0){
                        strAnswer.append("\n");
                    }
                    answer++;
                    strAnswer.append(size - 1).append(" ").append(size);
                }
                if (base[size - 1][size - 2].equals(j== 0 ? "0" : "1")){
                    if (answer > 0){
                        strAnswer.append("\n");
                    }
                    answer++;
                    strAnswer.append(size).append(" ").append(size - 1);
                }
                if (answer <= 2){
                    break;
                }
            ...
            writeInt(answer, "\n");
            writeString(strAnswer.toString(), "\n");
        }
        print();
    }
}
