...
public class D {
    ...
    static class Solver {
        ...
        int query(int c, int d) {
            ...
            System.out.flush();
            int ans = io.nextInt();
            //int ans = ((18^c) > (511^d) ? 1 : -1);
            if (ans == 0) ans++;
            return ans; // 1 means a>b or draw, -1 means b>a
        }

        void solve() {
            int leader = query(0,0);
            int ansA = 0;
            int ansB = 0;
            int carryA = 0;
            int carryB = 0;
            for (int i=2; i<32; i++) {
                int b = (1 << (31 - i));
                int ans11 = query(carryA|b, carryB|b);
                if (ans11 != leader) {
                    // Case: 11 flipped leader so it's either 10 or 01
                    // Let's use 2nd query to figure out who's leader after fixing this bit
                    if (leader == 1) {
                        // Case: a>=b , 10
                        ansA |= b;
                        carryB |= b;
                    ...
                    leader = query(carryA, carryB);
                } else {
                    // Case: 11 didn't flip leader so it's either 00 or 11
                    //      -> No need to fix bit
                    //      -> Leader doesn't change
                    // Let's use 2nd query to figure out whether this bit is 00 or 11
                    int ans10 = query(carryA|b, carryB);
                    if ((leader == 1 && ans10 != leader) || (leader == -1 && ans10 == leader)) {
                        // Case: 11
                        ansA |= b;
                        ansB |= b;
                    } // Case 00 doesn't require actions
                }
                //System.out.println("i="+i +", a="+Integer.toBinaryString(ansA) +", b="+Integer.toBinaryString(ansB));
            }

            io.println("! " + ansA + " " + ansB);
        }
