...
public class d {
    ...
    //   Student[] ss=new Student[m];
      HashMap<Integer,Integer> [] h=new HashMap[n+1];
       for(int i=0;i<m;i++){
           String[] sss=s.readLine().trim().split("\\s+");
            int u=Integer.parseInt(sss[0]);
            int v=Integer.parseInt(sss[1]);
            if(h[u]==null) h[u]=new HashMap<>();
           // if(h[v]==null) h[v]=new HashMap<>();
            h[u].put(v,1);//h[v].put(u,1);

       }int x=a[n-1];int ans=0;
       ArrayList<Integer> p=new ArrayList<>();
       for(int i=n-1;i>=0;i--){
           if(h[a[i]]==null||!h[a[i]].containsKey(x)){
               p.add(a[i]);
           }else{
               int flag=0;
               for(Integer j:p){
                   if(!h[a[i]].containsKey(j)){
                       flag=1;break;
                   }
               }
               if(flag==1){
