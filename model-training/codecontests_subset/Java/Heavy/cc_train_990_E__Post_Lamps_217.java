...
public class Main implements Runnable {
    ...
    void solve() {
        ...
        int m = in.nextInt();
        int k = in.nextInt();
        int[] block = in.nextIntArray(m);
        int[] cost = in.nextIntArray(k);
        if ((m > 0 && block[0] == 0) || m == n) {
            printf(-1);
            return;
        }

        boolean[] isBlocked = new boolean[n + 1];
        for (int i = 0; i < m; i++) isBlocked[block[i]] = true;
        int[] free = new int[n - m];
        int start = 0;
        for (int i = 0; i < n; i++) if (!isBlocked[i]) free[start++] = i;
        ...
        for (int power = 0; power < k; power++) {
            ...
            while (start < n - 1) {
                if (!isBlocked[start + 1]) {
                    ...
                    int pos = ArrayUtils.upperBound(free, start) - 1;
                    if (pos == -1 || free[pos] < start - power + 1) continue outter;
                    start = free[pos] + power;
                }
                cur += cost[power];
            }
    ...
    static class ArrayUtils {
        ...
        static int lowerBound(long[] a, int l, int r, long k) {
            int m = 0, ans = r;
            while (l < r) if (a[m = l + r >> 1] >= k) ans = r = m;
            else l = m + 1;
            return ans;
        ...
        static int lowerBound(long[] a, long k) {
            return lowerBound(a, 0, a.length, k);
        }

        static int upperBound(long[] a, int l, int r, long k) {
            int m = 0, ans = r;
            while (l < r) if (a[m = l + r >> 1] > k) ans = r = m;
            else l = m + 1;
            return ans;
        ...
        static int upperBound(long[] a, long k) {
            return upperBound(a, 0, a.length, k);
        }
