...
//        BufferedReader in = new BufferedReader(new InputStreamReader(System.in));
        ...
        for (int i=0;i<n + m;i++) h.add(DSU.findSet(i));
...
class Game
{
    ...
    public Game(long a, long b, long c, long d)
    {
        this.a = a;
        this.b = b;
        this.c = c;
...
{
    public int maxOfIntArray(int[] A)
    {
        int max = Integer.MIN_VALUE;
        for (int i=0;i<A.length;i++)    max = Math.max(max, A[i]);
        return max;
    }
    public long maxOfLongArray(long[] A)
    {
        long max = Long.MIN_VALUE;
        for (int i=0;i<A.length;i++)    max = Math.max(max, A[i]);
        return max;
    }
    public int minOfIntArray(int[] A)
    {
        int min = Integer.MAX_VALUE;
        for (int i=0;i<A.length;i++)    min = Math.min(min, A[i]);
        return min;
    }
    public long minOfLongArray(long[] A)
    {
