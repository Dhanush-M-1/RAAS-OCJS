...
while(tt-->0){
    ...
    if(l==end){
        ...
        long segRightLimit  = 2*(n-1);
        long segver = 1 ;
        long prevSegEnd = 0 ; 
        while(l>segRightLimit){
            segver ++ ;
            prevSegEnd = segRightLimit ;
            segRightLimit += 2*(n-segver);
        }
        // pn(segver);
        // pn(prevSegEnd);
        for(long j= prevSegEnd+1,segel=0;j<=r;++j){
            long content = 0;
            if(segel == (n-segver)){
                segel =0;
                segver++;
