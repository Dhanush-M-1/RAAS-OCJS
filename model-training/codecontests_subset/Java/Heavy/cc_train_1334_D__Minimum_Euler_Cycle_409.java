...
public class Main{
  ...
  static void solve(){
    ...
    long len=r-l+1;
    long[] ku=dokoku(l);
    System.err.println(ku[0]+":"+ku[1]);
    Kubun kubun;
    while(len>0){
      kubun=new Kubun((int)(ku[0]));
      kubun.setPos((int)(ku[1]));
      ku[1]=1;
      while(len>0&&kubun.hasNext()){
        out.print(kubun.next()+" ");
        len--;
      }
      ku[0]++;
    }
