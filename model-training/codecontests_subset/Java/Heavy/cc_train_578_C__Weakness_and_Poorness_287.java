...
public class C implements Runnable{
	...
	void solve() throws IOException {
		...
		for (int iteration = 0; iteration < ITERATIONS; ++iteration) {
			double mLeft = left + (right - left) / 3;
			double mRight = right - (right - left) / 3;
			
			double fLeft = f(n, a, mLeft);
			double fRight = f(n, a, mRight);
		...
		double ansX = (left + right) / 2;
		double ansWeakness = f(n, a, ansX);
		
		out.println(ansWeakness);
	}
	
	double f(int n, int[] a, double x) {
		double maxWeakness = 0;
		
		double minPrefixSum = 0, maxPrefixSum = 0;
		...
		for (int value : a) {
			...
			maxWeakness = max(maxWeakness, abs(prefixSum - minPrefixSum));
			maxWeakness = max(maxWeakness, abs(maxPrefixSum - prefixSum));
		}
