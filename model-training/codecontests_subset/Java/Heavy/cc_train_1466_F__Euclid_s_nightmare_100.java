...
class Solver {
    ...
    static final boolean TESTCASES = false;
    ...
    void solve(int tc) throws Exception {
        ...
        Set<int[]> edges = new TreeSet<>((a, b) -> Integer.compare(a[0], b[0]));
        for (i = 0; i < N; ++i) {
            int[] e;
            if (hp.nextInt() == 1) {
                e = new int[] {i, hp.nextInt() - 1, M};
            } else {
                e = new int[] {i, hp.nextInt() - 1, hp.nextInt() - 1};
            }
            edges.add(e);
        }


        // Thanks https://codeforces.com/contest/1466/submission/102844566 and https://codeforces.com/blog/entry/86126
        ...
        for (int[] itr : edges) {
            if (itr.length == 2) {
                if (single < 0) single = itr[1];
                else if (getRoot(single) == getRoot(itr[1])) rem.add(itr);
                else addEdge(single, itr[1]);
            } else if (itr.length == 3) {
                int a = getRoot(itr[1]), b = getRoot(itr[2]);
                if (a == b) rem.add(itr);
                else addEdge(a, b);
            }
        }
        ...
        hp.printsp(hp.pow(2, edges.size(), MOD));
        hp.println(edges.size());
        for (int[] itr : edges) hp.printsp(itr[0] + 1);
    }
