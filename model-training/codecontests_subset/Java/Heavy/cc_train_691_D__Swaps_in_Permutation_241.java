...
public class Main implements Runnable {
    ...
    void solve() {
        ...
        DSU dsu = new DSU(n + 1);
        for (int i = 0; i < m; i++) {
            dsu.unite(in.nextInt(), in.nextInt());
        }

        List<Integer>[] indexes = new ArrayList[n + 1];
        List<Integer>[] numbers = new ArrayList[n + 1];
        for (int i = 1; i <= n; i++) {
            int d = dsu.find(i);
            if (indexes[d] == null) {
                indexes[d] = new ArrayList<>();
                numbers[d] = new ArrayList<>();
            }
            indexes[d].add(i);
            numbers[d].add(p[i]);
        }
        ...
        for (int k = 1; k <= n; k++) {
            if (indexes[k] != null) {
                Collections.sort(numbers[k], Collections.reverseOrder());
                for (int i = 0; i < indexes[k].size(); i++) {
                    p[indexes[k].get(i)] = numbers[k].get(i);
                }
            }
        }

        printf(Arrays.stream(p).skip(1).toArray());
    }

    static class DSU {
        private int vertex, countCC;
        private int[] id, size;
        ...
        void unite(int i, int j) {
            ...
            countCC--;
            if (size[x] > size[y]) {
                id[y] = id[x]; size[x] += size[y]; size[y] = 0;
                return;
            }
            id[x] = id[y]; size[y] += size[x]; size[x] = 0;
        }
