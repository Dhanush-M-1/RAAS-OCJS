...
public class Solution {
	...
	static class Task {
		...
		public void solve(int testNumber, InputReader in, OutputWriter out) throws IOException {
			...
			int[] per = in.nextIntArray(n, 0);
			int[] keys = in.nextIntArray(k, 0);
			for (int i = 0; i < n; i++) per[i] -= p;
			for (int i = 0; i < k; i++) keys[i] -= p;
			Arrays.sort(per);
			Arrays.sort(keys);
			...
			for (int i = 0; i < k - n + 1; i++) {
				...
				for (int j = 0; j < n; j++) {
					long x = Math.abs(per[j] - keys[i + j]) + Math.abs(keys[i + j]);
					max = Math.max(max, x);
				}
