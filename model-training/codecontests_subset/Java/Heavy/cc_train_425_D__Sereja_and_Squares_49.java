...
//		try {
		int[] qtyX = new int[M + 1];
		int[] qtyY = new int[M + 1];
		for (int i = 0; i < count; i++) {
			qtyX[x[i]]++;
			qtyY[y[i]]++;
		}
		...
		for (int i = 0; i <= M; i++) {
			if (qtyX[i] != 0)
				byX[i] = new int[qtyX[i]];
			if (qtyY[i] != 0)
				byY[i] = new int[qtyY[i]];
		}
		Arrays.fill(qtyX, 0);
		Arrays.fill(qtyY, 0);
		for (int i = 0; i < count; i++) {
			byX[x[i]][qtyX[x[i]]++] = y[i];
			byY[y[i]][qtyY[y[i]]++] = x[i];
		}
		int answer = 0;
		for (int i = 0; i < count; i++) {
			int[] row = byX[x[i]];
			int[] column = byY[y[i]];
			int atX = Arrays.binarySearch(row, y[i]);
			int atY = Arrays.binarySearch(column, x[i]);
			if (row.length - atX > column.length - atY) {
				for (int j = atY + 1; j < column.length; j++) {
					int delta = column[j] - x[i];
					if (Arrays.binarySearch(row, y[i] + delta) >= 0 && Arrays.binarySearch(byX[column[j]], y[i] + delta) >= 0)
						answer++;
				}
			} else {
				for (int j = atX + 1; j < row.length; j++) {
					int delta = row[j] - y[i];
					if (Arrays.binarySearch(column, x[i] + delta) >= 0 && Arrays.binarySearch(byY[row[j]], x[i] + delta) >= 0)
						answer++;
				}
			}
		}
