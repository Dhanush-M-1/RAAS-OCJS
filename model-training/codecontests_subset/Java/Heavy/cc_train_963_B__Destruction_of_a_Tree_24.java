...
public class D {
    ...
    static class Solver {
        ...
        void solve() {
            ...
            Deque<IDval> quu = new ArrayDeque<>();
            //int root = 1;
            for (int i = 2; i <= n; i++) {
                //if (size[i] % 2==0)
                    //root = i;
            }
            quu.add(new IDval(root, 0));

            int[] depth = new int[n+1];
            boolean[] skip = new boolean[n+1];
            skip[root] = true;
            while (!quu.isEmpty()) {
                IDval curr = quu.pollFirst();
                int nextDepth = curr.val+1;
                for (int next : edges[curr.id]) {
                    if (skip[next]) continue;
                    skip[next] = true;
                    depth[next] = nextDepth;
                    quu.addLast(new IDval(next, nextDepth));
                }
            }

            List<Integer> ans = new ArrayList<>(n);
            boolean[] inq = new boolean[n+1];
            TreeSet<IDval> q = new TreeSet<>();
            for (int i=1; i<=n; i++) {
                if (size[i] % 2 == 0) {
                    inq[i] = true;
                    q.add(new IDval(i, depth[i]));
                }
            }
            boolean[] killed = new boolean[n+1];
            while (!q.isEmpty()) {
                IDval curr = q.pollLast();
                if (!inq[curr.id]) continue;
                if (killed[curr.id]) continue;
                killed[curr.id] = true;
                for (int next : edges[curr.id]) {
                    if (size[next] == 0) continue;
                    size[next]--;
                    if (size[next] % 2 == 0) {
                        q.add(new IDval(next, depth[next]));
                        inq[next] = true;
                    } else {
                        inq[next] = false;
                    }
            ...
            io.println("YES");
            boolean[] inAns = new boolean[n+1];
            for (int a : ans) {
                inAns[a] = true;
                io.println(a);
            }
            for (int i=1; i<=n; i++) {
                if (!inAns[i]) io.println(i);
            }
        }
