...
public class C {
	...
	SimpleAdjListGraph union(SimpleAdjListGraph... gs) {
		int V = 0;
		int E = 2;
		for(SimpleAdjListGraph gg : gs) {
			V += gg.V;
			E += gg.E;
		}
		SimpleAdjListGraph g = new SimpleAdjListGraph(V, E);
		int v0 = 0;
		for(SimpleAdjListGraph gg : gs) {
...
//		int TEST_CASE = Integer.parseInt(new String(io.nextLine()).trim());
		...
		while(TEST_CASE-- != 0) {
			...
			for(int i = 0, j; i < m; i = j) {
				...
				SimpleAdjListGraph g = new SimpleAdjListGraph(id, (j - i) * 2);
				for(j = i; j < m && es[i][2] == es[j][2]; j++) {
