...
//				pw.println(cur+" "+j+" "+" "+flag+" "+set[p[j]].contains(p[cur]));
				if(set[p[j]].contains(p[cur]))
				{
					int tmp = p[j+1];
					for(int k=j+1; k<=cur-1; k++)
					{
						if(set[p[j]].contains(p[k]))
						{
						//	p[k] = p[k+1];
...
//			pw.println();
	...
	public static int fun(long end, long start, int[] a)
	{
	
		if(end<start)
			return 0;
		
		return a[(int)end] - a[(int)start-1];
	}
