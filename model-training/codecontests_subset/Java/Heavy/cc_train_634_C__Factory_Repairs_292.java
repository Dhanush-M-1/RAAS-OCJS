...
{	
	...
	public void update(int d, int add, int k)
	{
		if(tr[k].left == tr[k].right)
		{
			if(c[d] < b)
			{
				tr[k].sum[0] += Math.min(b, c[d] + add) - c[d];
			}
			if(c[d] < a)
			{
				tr[k].sum[1] += Math.min(a, c[d] + add) - c[d];
			}
			return;
		}
		if(d <= tr[k << 1].right)
		...
		tr[k].sum[0] = tr[k << 1].sum[0] + tr[(k << 1) + 1].sum[0];
		tr[k].sum[1] = tr[k << 1].sum[1] + tr[(k << 1) + 1].sum[1];
	}
	
	public long query(int left, int right, int k, int type)
	{
		if(left <= tr[k].left && right >= tr[k].right)
		{
			return tr[k].sum[type];
		}
		long ans = 0;
		if(left <= tr[k << 1].right)
		{
			ans += query(left, right, k << 1, type);
		}
		if(right >= tr[(k << 1) + 1].left)
		{
			ans += query(left, right, (k << 1) + 1, type);
		}
		return ans;
	}
	...
	public void foo()
	{
		...
		while(q-- > 0)
		{
			...
			{
				int p = scan.nextInt();
				long ans = query(1, n, 1, 1) - query(1, p + k - 1, 1, 1);
				if(p >= 2)
				{
					ans += query(1, p - 1, 1, 0);
				}
	...
	class Node
	{
		...
		private int right;
		private long[] sum;
		public Node(int aLeft, int aRight)
		{
			left = aLeft;
			right = aRight;
			sum = new long[2];
