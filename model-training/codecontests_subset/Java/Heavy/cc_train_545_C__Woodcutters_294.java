...
public class Main {
    ...
    void solve() throws IOException {
        ...
        for (int i=0; i<n; ++i){
            trees[i] = new Tree(readInt(), readInt());
        }

        int prev = -1000*1000*1000-10000;
        int ans = 1;
        for (int i=0; i<n-1; ++i){
            if (trees[i].x - trees[i].height > prev){
                ans++;
                prev = trees[i].x;
            }else if (trees[i].x + trees[i].height< trees[i+1].x){
                prev = trees[i].x + trees[i].height;
                ans++;
            }else{
                prev = trees[i].x;
            }
