...
public class Main {
    ...
    static class TaskD {
        ...
        public void solve(int testNumber, InputReader in, OutputWriter out) {
            ...
            comp = ArrayUtils.createArray(n, -1);
            int compCount = 0;
            for (int i = 0; i < n; i++) {
                if (comp[i] == -1) {
                    dfs(i, compCount++);
            ...
            IntList[] fullComp = new IntList[compCount];
            for (int i = 0; i < compCount; i++) {
                fullComp[i] = new IntArrayList();
            }
            for (int i = 0; i < n; i++) {
                fullComp[comp[i]].add(i);
            }
        ...
        private void swap(int[] p, IntList comp) {
            int[] a = new int[comp.size()];
            for (int i = 0; i < comp.size(); i++) {
                a[i] = p[comp.get(i)];
            }
            Arrays.sort(a);
            ArrayUtils.sort(a, IntComparator.REVERSE);
            comp.sort();
            for (int i = 0; i < comp.size(); i++) {
                p[comp.get(i)] = a[i];
            }
