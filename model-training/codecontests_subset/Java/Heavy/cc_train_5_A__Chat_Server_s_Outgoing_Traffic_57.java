...
public class Main2 {
    ...
    public static void main(String[] args) {
        ...
        while (S.hasNextLine()) {
            ...
            if (curr.charAt(0) == '+') {
                users.add(curr.substring(1));
            } else if (curr.charAt(0) == '-') {
                users.remove(curr.substring(1));
            } else {
                int colonPos = curr.indexOf(':');
                int len = curr.length() - colonPos - 1;
                ans += len * users.size();
            }
        }
    ...
    static long[] extendEuclid(int a,int b){long[] ans=new long[2];if(b==0){ans[0]=1;ans[1]=0;return ans;}else{ans=extendEuclid(b,a%b);long x=ans[0],y=ans[1];ans[0]=y;ans[1]=x-((a/b)*y);return ans;}}
