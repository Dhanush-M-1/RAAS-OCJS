...
class TaskC {
    public void solve(int testNumber, InputReader in, OutputWriter out) {
		...
		int q = in.readInt();
		int[] order = new int[n];
		SumIntervalTree aTree = new SumIntervalTree(n);
		SumIntervalTree bTree = new SumIntervalTree(n);
		for (int it = 0; it < q; ++it) {
			int t = in.readInt();
			if (t == 1) {
				int di = in.readInt() - 1;
				int ai = in.readInt();
				aTree.update(di, di, -aTree.query(di, di));
				bTree.update(di, di, -bTree.query(di, di));
				order[di] += ai;
				aTree.update(di, di, Math.min(a, order[di]));
				bTree.update(di, di, Math.min(b, order[di]));
			} else {
				int from = in.readInt() - 1;
				int to = from + k;
				long ans = bTree.query(0, from - 1);
				ans += aTree.query(to, n - 1);
				out.printLine(ans);
			}
		}
    }
...
class SumIntervalTree extends LongIntervalTree {
	final int mod;
    public SumIntervalTree(int size, int mod) {
        super(size);
		this.mod = mod;
    }
    ...
    protected long joinValue(long left, long right) {
        return (mod == 0) ? (left + right) : (left + right) % mod;
    }

    protected long joinDelta(long was, long delta) {
        return (mod == 0) ? was + delta : (was + delta) % mod;
    }

    protected long accumulate(long value, long delta, int length) {
        return (mod == 0) ? value + delta * length : (value + delta * length) % mod;
    }
