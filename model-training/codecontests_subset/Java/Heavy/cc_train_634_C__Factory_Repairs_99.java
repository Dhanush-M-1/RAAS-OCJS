...
public	class Q2 {
			...
			public static void main(String[] args) throws IOException {
				...
				while(q-->0){
					...
					//	arr[d] = Math.min(arr[d]+v, a);
					//	brr[d] = Math.min(brr[d]+v, b);
			...
			static class SegmentTree{
				...
				{
				//	System.out.println(lazy[node]+" "+node);
				    if(lazy[node])
				    { 
				        // This node needs to be updated
				    	//int temp = st[node][0];
				    	//st[node][0] = st[node][1];
				    	//st[node][1] = temp; // Update it
				        if(start != end)
				        {
				    ...
				    st[node][1] = st[node*2][1] + st[node*2+1][1];// Updating root with max value 
				}
				int[] queryRange(int node, int start, int end, int l, int r)
				{
				    if(start > end || start > r || end < l)
				        return new int[]{0,0};         // Out of range
				    if(lazy[node])
				    {
				    	int temp = st[node][0];
				    	st[node][0] = st[node][1];
