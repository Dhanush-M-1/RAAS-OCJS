//package com.codeforces.competitions.year2016.round372div2;
...
/*					for (int i = 0; i < v; i++)
					{
						for (int curr : adj[i])
						{
							if (set[edges[curr].to].contains(i))
								continue;

							out.println(i + " " + edges[curr].to + " " + edges[curr].weight);
							set[i].add(edges[curr].to);
						}
					}*/
				}
			}
		...
		void dijkstra()
		{
			...
			while (set.size() > 0)
			{
				...
				for (int ed : adj[curr])
				{
					int to = edges[ed].from + edges[ed].to - curr;
