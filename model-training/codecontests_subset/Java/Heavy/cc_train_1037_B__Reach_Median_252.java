...
public class C1037B implements Runnable{
    public static void main(String[] args) {
	    new Thread(null, new C1037B(),"Main",1<<27).start();
    }
    @Override
    public void run() {
        ...
        Arrays.sort(data);
        //for(int a : data){out.print(a+" ");}
        long ans = 0;
        if(data[n/2] < k){
            for(int i = n/2; i < n; i++){
                ans += Math.max(k-data[i],0);
            }
        }
        else{
            for(int i = 0; i <= n/2; i++){
                ans += Math.max(data[i]-k,0);
            }
        }
        out.println(ans);
        out.close();
