...
public class C {
	...
	class SegmentTree {
		...
		public SegmentTree(int n, int upper) {
			this.upper = upper;
			N = Integer.highestOneBit(n) << 1;
			int size = (N << 1) - 1;
			sumDat = new long[size];
		}
		
		void sumUpdate(int k, long a) {
			k += N - 1;
			sumDat[k] = Math.min(sumDat[k] + a, upper);
			while (k > 0) {
				k = (k - 1) / 2;
		...
		private long sumQuery(int a, int b, int k, int l, int r) {
			if (r < a || b < l) return 0;
			
			if (a <= l && r <= b) return sumDat[k];
			else {
				long vl = sumQuery(a, b, k * 2 + 1, l, (l + r) / 2);
				long vr = sumQuery(a, b, k * 2 + 2, (l + r) / 2 + 1, r);
				return vl + vr;
			}
		}
		
		long getSumQuery(int a, int b) {
	...
	public void solve() {
		...
		for (int q = 0; q < Q; q++) {
			...
			if (type == 1) {
				...
				int p = in.nextInt();
				long res = st1.getSumQuery(0, p - 1);
				if (p + K <= N)
					res += st2.getSumQuery(p + K, N);
				out.println(res);
			}
		}
		out.close();
